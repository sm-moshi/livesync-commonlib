 ...
 */
export function isAcceptedAlwaysFactory(
	host: NecessaryServices<any, any>,
	log: LogFunction,
) {
	return (file: string | UXFileInfoStub) => {
		log(
			"File is target finally: " + getStoragePathFromUXFileInfo(file),
			LOG_LEVEL_DEBUG,
		);
...
export function isAcceptedInFilenameDuplicationFactory(
	...
	const fileCountMapComputed = new Computed({
		evaluation: (fileEventCount: number) => {
			const vaultFiles = host.serviceModules.storageAccess
				.getFileNames()
				.sort();
	...
	return async function isAcceptedInFilenameDuplication(
		...
		const fileCountMap = (
			await fileCountMapComputed.update(
...
export function isAcceptedByLocalDBFactory(
	...
	return async (file: string | UXFileInfoStub): Promise<boolean> => {
		...
		if (
			!(await Promise.resolve(database.localDatabase.isTargetFile(filepath)))
		) {
...
export function isAcceptedByIgnoreFilesFactory(
	...
	const getIgnoreFile = async (path: string): Promise<string[] | false> => {
		...
		try {
			if (
				!(await host.serviceModules.storageAccess.isExistsIncludeHidden(path))
			) {
			...
			const file =
				await host.serviceModules.storageAccess.readHiddenFileText(path);
	...
	return async function isAcceptedByIgnoreFiles(
		file: string | UXFileInfoStub,
	): Promise<boolean> {
		const settings = host.services.setting.currentSettings();
		...
		if (
			!(await isAcceptedAll(filepath, ignoreFiles, (filename) =>
