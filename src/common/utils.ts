...
import {
	...
	waitForSignal,
	waitForValue,
} from "octagonal-wheels/messagepassing/signal";
...
export function resolveWithIgnoreKnownError<T>(
	p: Promise<T>,
	def: T,
): Promise<T> {
...
export async function allSettledWithConcurrencyLimit<T>(
	processes: Promise<T>[],
	limit: number,
) {
	const ps = Parallels();
...
export function createBlob(
	data: string | string[] | Uint8Array<ArrayBuffer> | ArrayBuffer | Blob,
) {
...
export async function isDocContentSame(
	...
	while (i < length) {
		...
		i += checkQuantum;
		if (
			(await arrayBufferToBase64Single(ab1)) !=
			(await arrayBufferToBase64Single(ab2))
		)
			return false;
	}
	return true;
...
export function memorizeFuncWithLRUCacheMulti<T extends Array<any>, U>(
	func: (...keys: T) => U,
) {
 ...
 */
export async function runWithInterval<T>(
	key: string,
	interval: number,
	task: () => Promise<T>,
): Promise<T> {
 ...
 * @remarks This function is not designed to be concurrent.
 */
export async function runWithStartInterval<T>(
	key: string,
	interval: number,
	task: () => Promise<T>,
): Promise<T> {
...
export function isLoadedEntry(doc: DatabaseEntry): doc is LoadedEntry {
	return (
		"type" in doc &&
		(doc.type == "newnote" || doc.type == "plain") &&
		"data" in doc
	);
}
...
export function createSavingEntryFromLoadedEntry(
	doc: LoadedEntry,
): SavingEntry {
	...
	for (let i = 1; i < sorted.length; i++) {
		if (sorted[i] === end + 1) {
			...
			ranges.push(
				start === end
					? `${start.toString(32)}`
					: `${start.toString(32)}-${end.toString(32)}`,
			);
			start = sorted[i];
			end = sorted[i];
		}
...
export function applyPatch(
	from: Record<string | number | symbol, any>,
	patch: Record<string | number | symbol, any>,
) {
	const ret = from;
		...
		if (typeof value == "object") {
			...
			if (MARK_ISARRAY in value) {
				...
				if (!Array.isArray(ret[key])) {
					throw new Error(
						"Patch target type is mismatched (array to something)",
					);
				}
...
export function mergeObject(
	...
	const retSorted = Object.fromEntries(
		Object.entries(ret).sort((a, b) =>
 ...
 */
export function parseCustomRegExp(
	regexp: CustomRegExpSource,
): ParsedCustomRegExp {
...
function parseCustomRegExpList<D extends string>(
	list: CustomRegExpSourceList<D>,
	flags?: string,
	delimiter?: D,
) {
	const d = delimiter ?? ",";
	const items = list
		...
		.filter((e) => e);
	return items.map(
		(e) => new CustomRegExp(e as unknown as CustomRegExpSource, flags),
	);
}
...
export function constructCustomRegExpList<D extends string>(
	...
	return items
		.map((e) => `${e}`)
		.join(`${delimiter}`) as CustomRegExpSourceList<D>;
}
export function splitCustomRegExpList<D extends string>(
	list: CustomRegExpSourceList<D>,
	delimiter: D,
) {
	const d = delimiter;
	return list
		.split(d)
		.filter((e) => e as CustomRegExpSource) as CustomRegExpSource[];
}
...
export function getFileRegExp(
	settings: ObsidianLiveSyncSettings | RemoteDBSettings,
	key: RegExpSettingKey,
) {
 ...
 */
export function copyTo<T extends object, U extends T>(
	source: U,
	target: T,
): void {
...
export function pickBucketSyncSettings(
...
export function pickCouchDBSyncSettings(
	setting: ObsidianLiveSyncSettings,
): CouchDBConnection {
	return {
...
export function pickEncryptionSettings(
	setting: ObsidianLiveSyncSettings | EncryptionSettings,
): EncryptionSettings {
	return {
...
export function pickP2PSyncSettings(
	setting: Partial<ObsidianLiveSyncSettings> & P2PConnectionInfo,
): P2PConnectionInfo {
	return {
...
export function wrapByDefault<T, U>(
	func: () => T,
	onError: (err: Error) => U,
): T | U {
	try {
