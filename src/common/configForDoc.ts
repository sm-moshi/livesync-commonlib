...
export const DoctorRegulationV0_24_16: DoctorRegulation = {
	...
	rules: {
		sendChunksBulk: {
			value: false,
			reason:
				"This is an obsolete setting and we should not enable this no more",
		...
		doNotUseFixedRevisionForChunks: {
			value: true,
			reason:
				"This value has been reverted at v0.24.16 for garbage collection of chunks.",
			level: RuleLevel.Recommended,
		},
		handleFilenameCaseSensitive: {
			value: false,
			reason:
				"If Self-hosted LiveSync is Case-Sensitive, unexpected file operations may occur when synchronising with Windows, Android or other devices. This value should only be enabled if all devices have a Case-Sensitive file system.",
			requireRebuild: true,
			level: RuleLevel.Recommended,
		},
		useIndexedDBAdapter: {
		...
		hashAlg: {
			...
			reason:
				"The hash function is set to the old fallback. This should be retried. This may result in a change to the new fallback.",
		},
		disableCheckingConfigMismatch: {
			value: false,
			level: RuleLevel.Recommended,
			reason:
				"If you disabled an older version of the dialogue because it was hard to understand, try it once in the latest version.",
		},
		enableCompression: {
			value: false,
			level: RuleLevel.Recommended,
...
export const DoctorRegulationV0_24_30: DoctorRegulation = {
	...
	rules: {
		...
		usePluginSyncV2: {
			...
			level: RuleLevel.Recommended,
			reason:
				"This option is now enabled by default. If you have problems with the new plugin, please report them.",
		},
		chunkSplitterVersion: {
			...
			level: RuleLevel.Recommended,
			reason:
				"Chunk splitting has been optimised for more effective de-duplication. This is the new default value.",
		},
		customChunkSize: {
			min: 55,
			// max: 1000,
			...
			reason:
				"With the V3 Rabin-Karp chunk splitter and Self-hosted CouchDB, the chunk size is set to 60 (means around 6MB) by default. This is in effect the maximum chunk size, which in practice is divided more finely.",
		},
	},
};
...
export function checkUnsuitableValues(
	...
	for (const key in regulation.rules) {
		...
		if (rule.detectionFunc && !rule.detectionFunc(setting)) {
			Logger(
				`Rule condition satisfied: ${key} is ${value}, and detection function returned false`,
			);
			continue;
		}
		//@ts-ignore
...
export type RebuildOptionsType =
	(typeof RebuildOptions)[keyof typeof RebuildOptions];
...
export async function performDoctorConsultation(
	...
	if (issues.length == 0) {
		...
		const issueItems = Object.entries(r.rules) as [
			keyof ObsidianLiveSyncSettings,
			RuleForType<any>,
		][];
		...
		const OPTION_FIX_WITHOUT_REBUILD =
			`${$msg("Doctor.Button.FixButNoRebuild")}` as const;
		let skipped = 0;
		for (const [key, value] of issueItems) {
			...
			const options = [OPT_FIX] as [
				typeof OPT_FIX | typeof OPT_SKIP | typeof OPTION_FIX_WITHOUT_REBUILD,
			];
			...
			const ret = await env.confirm.askSelectStringDialogue(
				...
				options,
				{
					title: $msg("Doctor.Dialogue.TitleFix", {
						current: `${++idx}`,
