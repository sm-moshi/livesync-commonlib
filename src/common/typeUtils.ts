 ...
 * @returns
 */
export function isInternalMetadata(
	id: FilePath | FilePathWithPrefix | DocumentID,
): boolean {
	return id.startsWith(ICHeader);
}

export function isInternalFile(
	file: UXFileInfoStub | string | FilePathWithPrefix,
) {
	if (typeof file == "string")
		return isInternalMetadata(file as FilePathWithPrefix);
	if (file.isInternal) return true;
	return false;
}
export function stripInternalMetadataPrefix<
	T extends FilePath | FilePathWithPrefix | DocumentID,
>(id: T): T {
...
export function getPathFromUXFileInfo(
	file: UXFileInfoStub | string | FilePathWithPrefix,
) {
	if (typeof file == "string") return file as FilePathWithPrefix;
	return file.path;
}
export function getStoragePathFromUXFileInfo(
	file: UXFileInfoStub | string | FilePathWithPrefix,
) {
	if (typeof file == "string")
		return stripAllPrefixes(file as FilePathWithPrefix);
	return stripAllPrefixes(file.path);
}
export function getDatabasePathFromUXFileInfo(
	file: UXFileInfoStub | string | FilePathWithPrefix,
) {
	if (typeof file == "string" && file.startsWith(ICXHeader))
		return file as FilePathWithPrefix;
	const prefix = isInternalFile(file) ? ICHeader : "";
	if (typeof file == "string")
		return (prefix +
			stripAllPrefixes(file as FilePathWithPrefix)) as FilePathWithPrefix;
