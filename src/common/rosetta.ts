...
import {
	_allMessages,
	type MessageKeys,
} from "./messages/combinedMessages.dev"; // This sloppy-imports are used to replace the messages with the combined messages.
...
>(message: T, lang: I18N_LANGS, recurseLimit = 10): T {
	if (recurseLimit <= 0) {
		Logger(
	...
	const keywords = Object.entries(message)
		...
		.map(([key, value]) => [
			`${key.startsWith("K.") ? key.substring("K.".length) : key}`,
			value,
		])
		.map(
			([key, value]) =>
