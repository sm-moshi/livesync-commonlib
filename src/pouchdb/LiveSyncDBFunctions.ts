...
export async function ensureRemoteIsCompatible(
	...
	remoteMilestone.node_chunk_info = {
		...baseMilestone.node_chunk_info,
		...remoteMilestone.node_chunk_info,
	};
	let writeMilestone =
		remoteMilestone.node_chunk_info[deviceNodeID].min !=
			currentVersionRange.min ||
		remoteMilestone.node_chunk_info[deviceNodeID].max !=
			currentVersionRange.max ||
		isObjectDifferent(
			remoteMilestone.tweak_values?.[deviceNodeID],
			currentTweakValues,
		) ||
	...
	const diffLastConnected =
		now - (remoteMilestone.node_info[deviceNodeID].last_connected || 0);
	...
	if (!setting.disableCheckingConfigMismatch) {
		...
		const preferred_tweak =
			remoteMilestone.tweak_values?.[DEVICE_ID_PREFERRED] ?? currentTweakValues;
		const current_tweak = currentTweakValues as TweakValues;
		const preferred_should_matched = extractObject(
			TweakValuesShouldMatchedTemplate,
			{
				...TweakValuesDefault,
				...preferred_tweak,
		...
		const current_should_matched = extractObject(
			TweakValuesShouldMatchedTemplate,
			{
				...TweakValuesDefault,
				...current_tweak,
		...
		if (
			isObjectDifferent(preferred_should_matched, current_should_matched, true)
		) {
...
export async function ensureDatabaseIsCompatible(
	...
	const remoteMilestone = await resolveWithIgnoreKnownError<
		EntryMilestoneInfo | false
	>(db.get(MILESTONE_DOC_ID), false);
