...
export async function purgeUnreferencedChunks(
	...
	Logger(
		`Collecting unreferenced chunks on ${info.db_name}`,
		LOG_LEVEL_NOTICE,
	...
	if (chunks.length == 0) {
		Logger(
			`No unreferenced chunks! ${info.db_name}`,
			LOG_LEVEL_NOTICE,
			"gc-count-chunk" + keySuffix,
		);
	} else {
		...
		if (dryRun) {
			Logger(
				`DryRun of cleaning ${connSetting ? "remote" : "local"} database up: Done`,
				LOG_LEVEL_NOTICE,
			);
			return resultCount;
		}
		if (connSetting) {
			Logger(
				`Cleaning unreferenced chunks on remote`,
				LOG_LEVEL_NOTICE,
				"gc-purge" + keySuffix,
			);
			...
			Logger(
				`Cleaning unreferenced chunks on local`,
				LOG_LEVEL_NOTICE,
				"gc-purge" + keySuffix,
			);
		...
		Logger(
			`Cleaning unreferenced chunks done!`,
			LOG_LEVEL_NOTICE,
			"gc-purge" + keySuffix,
		);
	}
	...
	Logger(
		`Cleaning ${connSetting ? "remote" : "local"} database up: Done`,
		LOG_LEVEL_NOTICE,
	);
	return resultCount;
}
export function transferChunks(
	...
	return new QueueProcessor(
		async (batched) => {
			...
			const batch = local.rows
				.filter((e) => "error" in e && e.error == "not_found")
				.map((e) => e.key);
			return batch;
		},
		{
			...
			new QueueProcessor(
				async (docs) => {
					try {
						...
						Logger(
							`${label}: Something went wrong on balancing`,
							LOG_LEVEL_NOTICE,
						);
...
export async function balanceChunkPurgedDBs(
	local: PouchDB.Database,
	remote: PouchDB.Database,
) {
	Logger(`Complement missing chunks between databases`, LOG_LEVEL_NOTICE);
	try {
		const { onlyOnLocal, onlyOnRemote } = await collectUnbalancedChunkIDs(
			local,
			remote,
		);
		const localToRemote = transferChunks(
			"l2r",
			"local -> remote",
			local,
			remote,
		...
		const remoteToLocal = transferChunks(
			"r2l",
			"remote -> local",
			remote,
			local,
...
export async function fetchAllUsedChunks(
	local: PouchDB.Database,
	remote: PouchDB.Database,
) {
	try {
...
export async function purgeChunksLocal(
	db: PouchDB.Database,
	...
	await serialized("purge-local", async () => {
		try {
			...
			Logger(
				`Purging unused ${docs.length} chunks `,
				LOG_LEVEL_NOTICE,
				"purge-local-backup",
			);
			...
			for (const docsInBatch of batchDocsBackup) {
				...
				for (const item of erroredItems) {
					Logger(
						`Failed to back up: ${item.id} / ${item.rev}`,
						LOG_LEVEL_VERBOSE,
					);
				}
			}
		...
		for (const docsInBatch of batchDocs) {
			...
			const removed = await db.purgeMulti(
				docsInBatch.map((e) => [e.id, e.rev]),
			);
			const removedCount = Object.values(removed).filter(
				(e) => "ok" in (e as object),
			).length;
			totalRemoved += removedCount;
			Logger(
				`Purging:  ${totalRemoved} / ${docs.length}`,
				LOG_LEVEL_NOTICE,
				"purge-local",
			);
		}
...
export async function collectUnbalancedChunkIDs(
	local: PouchDB.Database,
	remote: PouchDB.Database,
) {
	const chunksOnLocal = await collectChunks(local, "INUSE");
	const chunksOnRemote = await collectChunks(remote, "INUSE");
	const onlyOnLocal = chunksOnLocal.filter(
		(e) => !chunksOnRemote.some((ee) => ee.id == e.id),
	);
	const onlyOnRemote = chunksOnRemote.filter(
		(e) => !chunksOnLocal.some((ee) => ee.id == e.id),
	);
	return { onlyOnLocal, onlyOnRemote };
}
export async function collectChunks(
	db: PouchDB.Database,
	type: "INUSE" | "DANGLING" | "ALL",
) {
	...
	const items = docs
		.filter((e) => !("error" in e))
		.map((e: any) => ({ id: e.id, rev: e.value.rev }));
	return items;
}
...
export async function collectChunksUsage(db: PouchDB.Database) {
	...
	const q = await db.query("chunks/collectDangling", {
		reduce: true,
		group: true,
	});
...
export async function purgeChunksRemote(
	setting: CouchDBConnection,
	docs: { id: string; rev: string }[],
) {
