...
type EncryptionVersion =
	(typeof EncryptionVersions)[keyof typeof EncryptionVersions];
...
async function encryptMetaWithHKDF<T extends AnyEntry>(
	...
	const encryptedMeta = await encryptHKDFWorker(
		propStr,
		passphrase,
		pbkdf2Salt,
	);
...
const MESSAGE_FALLBACK_DECRYPT_FAILED =
	"Failed to decrypt the data with V1 method. Cannot encrypt with HKDF.";
	...
	if (isEncryptedChunkEntry(saveDoc) || isSyncInfoEntry(saveDoc)) {
		try {
			...
			if (encryptionVersion === EncryptionVersions.ENCRYPTED) {
				const decrypted = await tryDecryptV1AsFallback(
					saveDoc.data,
					passphrase,
		...
		try {
			saveDoc.eden = {
				[EDEN_ENCRYPTED_KEY_HKDF]: {
					data: await encryptHKDF(
						JSON.stringify(saveDoc.eden),
						passphrase,
						pbkdf2salt,
					),
					epoch: 999999,
...
async function outgoingDecryptHKDF(
	...
	if (isEncryptedChunkEntry(loadDoc) || isSyncInfoEntry(loadDoc)) {
		try {
			...
			if (encryptionVersion === EncryptionVersions.HKDF) {
				...
				const decryptedData = await tryDecryptV1AsFallback(
					loadDoc.data,
					passphrase,
					useDynamicIterationCount,
				);
	...
	if (isObfuscatedEntry(loadDoc)) {
		...
		if (isEncryptedMeta(loadDoc)) {
			const pbkdf2salt = await getPBKDF2Salt();
			try {
				const metadata = await decryptMetaWithHKDF(
					path,
					passphrase,
					pbkdf2salt,
				);
			...
			const decryptedPath = await tryDecryptV1AsFallback(
				path,
				passphrase,
				useDynamicIterationCount,
			);
...
async function outgoingDecryptV1(
	...
	const _isChunkOrSyncInfo =
		isEncryptedChunkEntry(loadDoc) || isSyncInfoEntry(loadDoc);
	...
	if (_isChunkOrSyncInfo || _isObfuscatedEntry || _shouldDecryptEden) {
		...
		try {
			if (_isChunkOrSyncInfo) {
				...
				(loadDoc as any).data = await decrypt(
					(loadDoc as any).data,
					passphrase,
					useDynamicIterationCount,
				);
			...
			if (useDynamicIterationCount) {
				try {
					...
					Logger(ex, LOG_LEVEL_VERBOSE);
					Logger(
						`id:${loadDoc._id}-${loadDoc._rev?.substring(0, 10)}`,
						LOG_LEVEL_VERBOSE,
					);
					throw ex;
				}
...
export let preprocessOutgoing: (
	doc: AnyEntry | EntryLeaf,
) => Promise<AnyEntry | EntryLeaf> = async (doc) => {
	return await Promise.resolve(doc);
};
export let preprocessIncoming: (doc: EntryDoc) => Promise<EntryDoc> = async (
	doc,
) => {
	return await Promise.resolve(doc);
};
...
export function shouldEncryptEdenHKDF(
	doc: AnyEntry | EntryLeaf,
): doc is AnyEntry {
...
export function shouldDecryptEdenHKDF(
	doc: AnyEntry | EntryLeaf,
): doc is AnyEntry {
	if ("eden" in doc && EDEN_ENCRYPTED_KEY_HKDF in doc.eden) {
		return true;
