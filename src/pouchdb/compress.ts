...
export async function _compressText(text: string) {
	...
	const df = await wrappedDeflate(new Uint8Array(data), {
		consume: true,
		level: 8,
	});
	...
	const deflateResult =
		(converted ? "~" : "") + (await arrayBufferToBase64Single(df));
	return deflateResult;
}
export const wrappedInflate = wrapFflateFunc<
	Uint8Array<ArrayBuffer>,
	fflate.AsyncInflateOptions
>(fflate.inflate);
export const wrappedDeflate = wrapFflateFunc<
	Uint8Array<ArrayBuffer>,
	fflate.AsyncDeflateOptions
>(fflate.deflate);
...
export async function decompressDoc(doc: EntryDoc) {
	...
	if (doc.data.startsWith(MARK_SHIFT_COMPRESSED)) {
		doc.data = await _decompressText(
			doc.data.substring(MARK_SHIFT_COMPRESSED.length),
		);
	}
	return doc;
}
...
export const replicationFilter = (
