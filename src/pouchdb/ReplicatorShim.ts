...
export type SomeDocument<T extends object> = PouchDB.Core.ExistingDocument<T> &
...
export type PouchDBShim<T extends object> = {
	...
	) => Promise<PouchDB.Core.ChangesResponse<T>>;
	revsDiff: (
		diff: PouchDB.Core.RevisionDiffOptions,
	) => Promise<PouchDB.Core.RevisionDiffResponse>;
	bulkDocs: (
	...
	) => Promise<PouchDB.Core.BulkGetResponse<T>>;
	put: (
		doc: PouchDB.Core.PutDocument<any>,
...
type CompatibleDatabase<T extends object> =
	| PouchDB.Database<SomeDocument<T>>
	| PouchDBShim<SomeDocument<T>>;
export async function upsert<
	V extends object,
	T extends SomeDocument<V> = SomeDocument<V>,
>(db: CompatibleDatabase<object>, id: string, func: (doc: T) => T): Promise<T> {
	try {
...
export type ShimReplicationOption =
	| ShimReplicationOneShot
	| ShimReplicationOptionContinuous;
...
export async function replicateShim<
	T extends CompatibleDatabase<V>,
	U extends CompatibleDatabase<V>,
	V extends object,
>(
	targetDB: T,
	sourceDB: U,
	...
	try {
		...
		await serialized(
			`replication-${targetDBInfo.db_name}-${sourceDBInfo.db_name}`,
			async () => {
...
Target: ${targetDBInfo.db_name} (${targetDBInfo.update_seq})`;
				...
				const sourceCheckpointData = await upsert<{ mark: string }>(
					sourceDB,
					sourceCheckpointID,
					(doc) => {
						const previousMark: string =
							doc.mark ?? new Date().getTime().toString();
						const mark = option.rewind
				...
				Logger(
					`Replication from ${sourceDBName} to ${targetDBName} / since ${since}`,
					LOG_LEVEL_VERBOSE,
				);
				...
				do {
					...
					const diffCheckParamA = changesResults.map(
						(e) => [e.id, e.changes.map((e) => e.rev)] as const,
					);
					...
					const request = missing
						.map(([id, revs]) => revs.map((rev) => ({ id, rev })))
						.flat();
					...
					if (request.length !== 0) {
						...
						const _ = await targetDB.bulkDocs(fetchedMissingDocs, {
							new_edits: false,
						});
						...
						const processedDocs = docWithSeq
						...
						// }
						//     , "");
						const allSecs = Number.parseInt(
							`${changes.last_seq}`.split("-")[0],
						);
						try {
					...
					await upsert<{ since: string | number }>(
						targetDB,
						targetCheckpointID,
						(doc) => {
							return {
