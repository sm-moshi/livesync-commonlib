...
export type ChunkRetrievalResult =
	| ChunkRetrievalResultSuccess
	| ChunkRetrievalResultError;
export interface LiveSyncLocalDBEnv {
	...
	services: Pick<
		IServiceHub,
		"database" | "databaseEvents" | "replicator" | "setting"
	>;
}
...
export class LiveSyncLocalDB {
	...
	async initializeDatabase(): Promise<boolean> {
		...
		this.localDatabase =
			this.env.services.database.createPouchDBInstance<EntryDoc>(
		...
		if (
			!(await this.env.services.databaseEvents.onDatabaseInitialisation(this))
		) {
			Logger(
				"Initializing Database has been failed on some module",
				LOG_LEVEL_NOTICE,
			);
		...
		Logger(
			JSON.stringify(await this.localDatabase.info(), null, 2),
			LOG_LEVEL_VERBOSE,
		);
		...
		const unload = eventHub.onEvent(
			REMOTE_CHUNK_FETCHED,
			(chunk: EntryLeaf) => {
		...
		do {
			...
			for (const change of changes.results) {
				...
				if ("children" in doc) {
					...
					if (doc._conflicts) {
						const revs = await this.localDatabase.get(doc._id, {
							revs: true,
							revs_info: true,
						});
						...
						for (const conflict of doc._conflicts) {
							...
							const diffRevs = mineRevInfo.filter(
								(e) =>
									!conflictedRevInfo.some(
										(f) => f.rev == e.rev && f.status == e.status,
									),
							);
	...
	async resetDatabase() {
		...
		if (!(await this.env.services.databaseEvents.onResetDatabase(this))) {
			Logger(
				"Database reset has been prevented or failed on some modules.",
				LOG_LEVEL_NOTICE,
			);
			return false;
		}
	...
	async removeRevision(docId: DocumentID, revision: string): Promise<boolean> {
		try {
			...
			if (isErrorOfMissingDoc(ex)) {
				Logger(
					`Remove revision: Missing target revision, ${docId}-${revision}`,
					LOG_LEVEL_VERBOSE,
				);
			}
		}
	...
	async getDBEntryMeta(
		...
		return await this.managers.entryManager.getDBEntryMeta(
			path,
			opt,
			includeDeleted,
		);
	...
	async getDBEntry(
		...
		return await this.managers.entryManager.getDBEntry(
			path,
			opt,
			dump,
			waitForReady,
	...
	async getDBEntryFromMeta(
		...
		return await this.managers.entryManager.getDBEntryFromMeta(
			meta,
			dump,
			waitForReady,
		);
	...
	async getConflictedDoc(
		path: FilePathWithPrefix,
		rev: string,
	): Promise<false | diff_result_leaf> {
