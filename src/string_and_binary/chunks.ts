...
function* pickPiece(
	...
	L1: do {
		...
		if (
			...
			buffer += curLine + (leftData.length != 0 ? "\n" : "");
			if (
				buffer.length >= minimumChunkSize ||
				leftData.length == 0 ||
				leftData[0] == "#" ||
				buffer[0] == "#"
			) {
...
const segmenter =
	"Segmenter" in Intl
		? wrapByDefault(
				...
				(err) => {
					Logger(
						`Failed to create Intl.Segmenter: ${err.message}`,
						LOG_LEVEL_VERBOSE,
					);
					return undefined;
				},
...
export function splitPiecesTextV2(
...
export function binaryTextSplit(
	data: string,
	pieceSize: number,
	minimumChunkSize: number,
) {
...
function* chunkStringGeneratorFromGenerator(
	sources: Generator<string, void, unknown>,
	maxLength: number,
) {
	for (const source of sources) {
...
export async function collectGenAll(
	strGen: AsyncGenerator<string, any, unknown> | Generator<string>,
) {
	const ret = [] as string[];
...
export async function concatGeneratedAll(
	strGen: AsyncGenerator<string, any, unknown> | Generator<string>,
) {
	...
	return async function* piecesBlob(): AsyncGenerator<string> {
		...
		do {
			...
			const currentData = new Uint8Array(
				await dataSrc.slice(i, i + pieceSize).arrayBuffer(),
			);
			// Find null (or / at PDF) or newLine, and make chunks.
			// To avoid making too much chunks, all chunks should be longer than minimumChunkSize.
			// However, we might have been capped the chunk size due to HTTP request size or document size on CouchDB.
			...
			if (nextIdx == -1)
				nextIdx = currentData.indexOf(charNewLine, minimumChunkSize);
...
export async function splitPiecesRabinKarp(
	...
	const avgChunkSize = Math.max(
		minPieceSize,
		Math.floor(dataSrc.size / splitPieceCount),
	);
	const maxChunkSize = Math.min(absoluteMaxPieceSize, avgChunkSize * 5);
	const minChunkSize = Math.min(
		Math.max(Math.floor(avgChunkSize / 4), minimumChunkSize),
		maxChunkSize,
	);
	const windowSize = 48;
			...
			if (isBoundaryCandidate) {
				...
				if (isSafeBoundary) {
					if (isText) {
						...
						yield await arrayBufferToBase64Single(
							buffer.subarray(start, pos + 1),
						);
					}
					start = pos + 1;
				}
