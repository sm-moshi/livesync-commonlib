...
export function expandFilePathPrefix(
...
export function expandDocumentIDPrefix(
	id: DocumentID,
): [string, FilePathWithPrefix] {
...
export async function path2id_base(
	...
	if (filenameSrc.startsWith(PREFIX_OBFUSCATED))
		return `${filenameSrc}` as DocumentID;
...
const matchOpts: MinimatchOptions = {
	platform: "linux",
	dot: true,
	flipNegate: true,
	nocase: true,
	...
	for (const intermediatePath of intermediatePaths) {
		for (const ignoreFile of ignoreFiles) {
			...
			const result = isAccepted(
				path.substring(
					intermediatePath.length ? intermediatePath.length + 1 : 0,
				),
				list,
			);
			if (result !== undefined) {
