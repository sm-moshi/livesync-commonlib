...
export type RequiredServiceModules<T extends keyof ServiceModules> = Pick<
	ServiceModules,
	T
>;

export type NecessaryServices<
	T extends keyof ServiceHub,
	U extends keyof ServiceModules,
> = {
	services: RequiredServices<T>;
	serviceModules: RequiredServiceModules<U>;
};

export type ServiceFeatureFunction<
	T extends keyof ServiceHub,
	U extends keyof ServiceModules,
	TR,
> = (host: NecessaryServices<T, U>) => TR;
type ServiceFeatureContext<T> = T & {
	_log: LogFunction;
};
export type ServiceFeatureFunctionWithContext<
	T extends keyof ServiceHub,
	U extends keyof ServiceModules,
	C,
	TR,
> = (host: NecessaryServices<T, U>, context: ServiceFeatureContext<C>) => TR;
...
type ContextFactory<
	...
	U extends keyof ServiceModules,
	C,
> = (host: NecessaryServices<T, U>) => ServiceFeatureContext<C>;
...
>() {
	return {
		...
		withContext<C extends object = object>(
			ContextFactory: ContextFactory<T, U, C>,
		) {
			return {
				create:
					<TR>(
						featureFunction: ServiceFeatureFunctionWithContext<T, U, C, TR>,
					) =>
