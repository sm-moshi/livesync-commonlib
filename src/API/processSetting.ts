...
export function encodeSettingsToQRCodeData(settings: ObsidianLiveSyncSettings) {
	const settingArr = [];
	const fullIndexes = Object.entries(KeyIndexOfSettings) as [
 ...
 */
export function decodeSettingsFromQRCodeData(
	qr: string,
...
export function encodeQR(settingString: string, format: OutputFormat) {
	...
	try {
		...
		Logger(
			`Failed to encode QR Code (${(ex as any)?.message || String(ex)})`,
			LOG_LEVEL_NOTICE,
		);
...
export async function encodeSettingsToSetupURI(
	...
	const encryptedSetting = encodeURIComponent(
		await encryptString(JSON.stringify(setting), passphrase),
	);
...
export async function decodeSettingsFromSetupURI(
	...
	const decrypted = await decryptString(
		decodeURIComponent(encryptedSetting),
		passphrase,
	);
	try {
