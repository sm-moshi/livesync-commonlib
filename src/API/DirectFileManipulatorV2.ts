...
} from "../pouchdb/LiveSyncLocalDB.ts";
...
export class DirectFileManipulator implements LiveSyncLocalDBEnv {
	...
	getBoundDatabaseService(options: () => DirectFileManipulatorOptions) {
		...
		return class HeadlessDatabaseServiceExt<
	...
	constructor(options: DirectFileManipulatorOptions) {
		...
		// );
		(
			this.services.setting as InjectableSettingService<ServiceContext>
		).saveData.setHandler((data) => {
			console.warn(
				"Saving settings is not supported in DirectFileManipulator.",
			);
		...
		(
			...
			console.warn(
				"Loading settings is not supported in DirectFileManipulator.",
			);
		...
		this.services.databaseEvents.onDatabaseInitialisation.addHandler(
			this.$everyOnInitializeDatabase.bind(this),
		);
	...
	async getSyncParameters(setting: RemoteDBSettings): Promise<SyncParameters> {
		try {
			const downloadedSyncParams = await this.rawGet<SyncParameters>(
				DOCID_SYNC_PARAMETERS,
			);
			if (!downloadedSyncParams) {
				throw new SyncParamsNotFoundError(
					`Sync parameters have not been found in the database.`,
				);
			}
	...
	async putSyncParameters(
		...
		try {
			const ret = await this.liveSyncLocalDB.putRaw(
				params as unknown as EntryDoc,
			);
			if (ret.ok) {
				return true;
			}
	...
	async getReplicationPBKDF2Salt(
		setting: RemoteDBSettings,
		refresh?: boolean,
	...
	$everyOnInitializeDatabase(db: LiveSyncLocalDB): Promise<boolean> {
		...
		if (
			...
			typeof this.options.passphrase === "string"
		) {
			enableEncryption(
				db.localDatabase,
				this.options.passphrase,
	...
	get settings() {
		const retObj: RemoteDBSettings = {
			...
			...{
				minimumChunkSize:
					this.options.minimumChunkSize ?? DEFAULT_SETTINGS.minimumChunkSize,
				encrypt: this.options.passphrase ? true : false,
				passphrase: this.options.passphrase ?? "",
				deleteMetadataOfDeletedFiles:
					DEFAULT_SETTINGS.deleteMetadataOfDeletedFiles,
				customChunkSize:
					this.options.customChunkSize ?? DEFAULT_SETTINGS.customChunkSize,
				...
				maxChunksInEden:
					this.options.maxChunksInEden ?? DEFAULT_SETTINGS.maxChunksInEden,
				maxTotalLengthInEden:
					this.options.maxTotalLengthInEden ??
					DEFAULT_SETTINGS.maxTotalLengthInEden,
				maxAgeInEden:
					this.options.maxAgeInEden ?? DEFAULT_SETTINGS.maxAgeInEden,
				enableChunkSplitterV2:
					this.options.enableChunkSplitterV2 ??
					DEFAULT_SETTINGS.enableChunkSplitterV2,
				chunkSplitterVersion:
					this.options.chunkSplitterVersion ??
					DEFAULT_SETTINGS.chunkSplitterVersion,
				disableWorkerForGeneratingChunks: true,
				processSmallFilesInUIThread: true,
				doNotUseFixedRevisionForChunks:
					this.options.doNotUseFixedRevisionForChunks ??
					DEFAULT_SETTINGS.doNotUseFixedRevisionForChunks,
				...
				endpoint: "",
				enableCompression:
					this.options.enableCompression ?? DEFAULT_SETTINGS.enableCompression,
				handleFilenameCaseSensitive:
					this.options.handleFilenameCaseSensitive ??
					DEFAULT_SETTINGS.handleFilenameCaseSensitive,
	...
	async getById(
		id: string,
		metaOnly = false,
	): Promise<false | MetaEntry | ReadyEntry> {
	...
	async getByMeta(doc: MetaEntry): Promise<ReadyEntry> {
		const docX = await this.liveSyncLocalDB.getDBEntryFromMeta(
			doc as LoadedEntry,
		);
	...
	async delete(path: string) {
		...
		const ret = await this.liveSyncLocalDB.deleteDBEntry(
	...
	async *_enumerate(
		startKey: string,
		endKey: string,
		opt: { metaOnly: boolean },
	) {
		if (opt.metaOnly)
			return this.liveSyncLocalDB.findEntries(startKey, endKey, {});
		for await (const f of this.liveSyncLocalDB.findEntries(
			startKey,
			endKey,
			{},
		)) {
			...
			.on("change", async (change: any) => {
				...
				if (checkIsInterested) {
					if (!checkIsInterested(doc)) {
						Logger(
							`WATCH: SKIP ${doc.path}: OUT OF TARGET FOLDER`,
							LOG_LEVEL_VERBOSE,
							"watch",
						);
						return;
			...
			const last = await this.liveSyncLocalDB.localDatabase
				...
				.on("change", async (change) => {
					...
					if (checkIsInterested) {
						if (!checkIsInterested(doc)) {
							Logger(
								`FOLLOW: SKIP ${doc._id}: OUT OF TARGET FOLDER`,
								LOG_LEVEL_VERBOSE,
								"watch",
							);
							return;
