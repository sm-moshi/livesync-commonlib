...
function createSyncParamsHandler({
	put,
	get,
	create,
}: CreateSyncParamsHanderOptions): SyncParamsHandler {
	// To cache the fetched synchronisation parameters, we re-use the promise to save memory consumption.
	let taskFetchParameters:
		| Promise<SyncParametersWithDecoded | false>
		| undefined = undefined;

	// This function fetches synchronisation parameters from the server, ensuring that they have PBKDF2 salt.
	const _fetchSyncParameters = async (): Promise<
		SyncParametersWithDecoded | false
	> => {
		...
		try {
			...
			do {
				...
				try {
					...
					if (LiveSyncError.isCausedBy(ex, SyncParamsNotFoundError)) {
						...
						Logger(
							`Synchronisation parameters not found, creating new ones`,
							LOG_LEVEL_INFO,
						);
						...
						if (!putResult) {
							Logger(
								`Failed to store initial synchronisation parameters`,
								LOG_LEVEL_INFO,
							);
			...
			if (!syncParams) {
				throw new SyncParamsFetchError(
					`Unexpected empty synchronisation parameters`,
				);
			}
			// 2.2. If the synchronisation parameters do not have PBKDF2 salt, create a new one.
			if (!syncParams.pbkdf2salt) {
				Logger(
					`Synchronisation parameters do not have PBKDF2 salt, generating a new salt`,
					LOG_LEVEL_INFO,
				);
				...
				if (!putResult) {
					Logger(
						`Failed to store synchronisation parameters with new PBKDF2 salt`,
						LOG_LEVEL_INFO,
					);
					throw new SyncParamsUpdateError(
						`Failed to store synchronisation parameters with new PBKDF2 salt`,
			...
			if (!syncParams) {
				...
				throw new Error(
					`Failed to prepare synchronisation key in synchronisation parameters`,
				);
			}
			...
			if (!syncParams.pbkdf2saltDecoded) {
				const decodedSalt = new Uint8Array(
					base64ToArrayBufferInternalBrowser(syncParams.pbkdf2salt),
				);
				if (!decodedSalt) {
	...
	return {
		...
		getPBKDF2Salt: async (refresh: boolean = false) => {
			...
			if (!syncParams) {
				Logger(`Failed to fetch synchronisation parameters`, LOG_LEVEL_INFO);
				throw new SyncParamsFetchError(
					`Failed to fetch synchronisation parameters`,
				);
			}
