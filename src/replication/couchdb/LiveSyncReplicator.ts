...
import {
	checkRemoteVersion,
	countCompromisedChunks,
} from "../../pouchdb/negotiation.ts";
...
import {
	serialized,
	shareRunningResult,
} from "octagonal-wheels/concurrency/lock";
...
async function* genReplication(
	s:
		| PouchDB.Replication.Sync<EntryDoc>
		| PouchDB.Replication.Replication<EntryDoc>,
	signal: AbortSignal,
) {
	...
	try {
		while (!inbox.isDisposed && !signal.aborted) {
			const r = await inbox.pick(undefined, [abortPromise.promise] as Promise<
				typeof abortSymbol
			>[]);
			if (r === NOT_AVAILABLE) {
				break;
...
export class LiveSyncCouchDBReplicator extends LiveSyncAbstractReplicator {
	...
	async getSyncParameters(setting: RemoteDBSettings): Promise<SyncParameters> {
		try {
			const downloadedSyncParams =
				await this.fetchRemoteDocument<SyncParameters>(
					setting,
					DOCID_SYNC_PARAMETERS,
				);
			if (!downloadedSyncParams) {
				throw new SyncParamsNotFoundError(
					`Sync parameters not found on remote server`,
				);
			}
	...
	async putSyncParameters(
		...
		try {
			...
			if (ret.ok) {
				...
				throw new SyncParamsUpdateError(
					`Could not store remote sync parameters: ${JSON.stringify(ret)}`,
				);
			}
		} catch (ex) {
	...
	async replicationChangeDetected(
		...
		try {
			if (e.direction == "pull") {
				await this.env.services.replication.parseSynchroniseResult(
					e.change.docs,
				);
	...
	> {
		...
		try {
			for await (const [type, e] of gen) {
				// Pacing replication.
				const releaser = await globalConcurrencyController.tryAcquire(
					1,
					REPLICATION_BUSY_TIMEOUT,
				);
				if (releaser === false) {
					Logger(
						"Replication stopped for busy.",
						showResult ? LOG_LEVEL_NOTICE : LOG_LEVEL_INFO,
						"sync",
					);
					return "FAILED";
				...
				switch (type) {
					case "change":
						if ("direction" in e) {
							if (e.direction == "pull") {
								this.lastSyncPullSeq = Number(
									`${e.change.last_seq}`.split("-")[0],
								);
							} else {
								this.lastSyncPushSeq = Number(
									`${e.change.last_seq}`.split("-")[0],
								);
							}
						...
						if (retrying) {
							if (
								this.docSent -
									docSentOnStart +
									(this.docArrived - docArrivedOnStart) >
	...
	async getLastTransferredSeqOfChunks(
		...
		const previous_max_seq_on_chunk = await wrapException(() =>
			localDB.get<typeof prevMax>(prevMax._id),
		);
	...
	async updateMaxTransferredSeqOnChunks(
		...
		const previous_max_seq_on_chunk = await wrapException(() =>
			localDB.get<typeof newMax>(newMax._id),
		);
		if (previous_max_seq_on_chunk instanceof Error) {
			delete newMax._rev;
		} else {
			...
			newMax.seqStatusMap = {
				...previous_max_seq_on_chunk.seqStatusMap,
				...seqStatusMap,
			};
		}
	...
	override async sendChunks(
		...
		const trench = new Trench(
			this.env.services.keyValueDB.openSimpleStore<{
				seq: string | number;
				doc:
					| PouchDB.Core.ExistingDocument<EntryDoc & PouchDB.Core.ChangesMeta>
					| undefined;
				id: DocumentID;
			}>("sc-"),
			false,
		...
		if (!remoteDB) {
			const d = await this.connectRemoteCouchDBWithSetting(
				setting,
				this.isMobile(),
				true,
			);
		...
		const idSeqMap = Object.fromEntries(
			localScannedDocs.map((e) => [e.id, e.seq]),
		);
		for (const checkDocs of arrayToChunkedArray(
			localScannedDocs,
			limitOneBatch,
		)) {
			const remoteDocs = await remoteDB.allDocs({
				keys: checkDocs.map((e) => e.id),
				include_docs: false,
			});
			const remoteDocMap = Object.fromEntries(
				remoteDocs.rows.map((e) => [e.key, "error" in e ? e.error : e.value]),
			);
			const sendDocs = checkDocs.filter(
				(e) => remoteDocMap[e.id] == "not_found",
			);
			const sentDocs = checkDocs.filter(
				(e) => remoteDocMap[e.id] != "not_found",
			);
			sendDocs.forEach((e) => (sentMap[e.seq] = false));
			sentDocs.forEach((e) => (sentMap[e.seq] = true));
			const sendDocsMap = Object.fromEntries(
				sendDocs.map((e) => [e.id, e.seq]),
			);
			if (sendDocs.length > 0) {
				const localDocs = await localDB.allDocs({
					keys: sendDocs.map((e) => e.id),
					include_docs: true,
				});
				await trench.queue(
					"send-chunks",
		...
		const sendChunks = async (
			...
			try {
				...
				const uploadBulkDocs = (await Promise.all(
					uploadBulkDocTasks,
				)) as EntryLeaf[];
		...
		do {
			const nowSendChunks =
				await trench.dequeue<
					{
						seq: string | number;
						doc:
							| PouchDB.Core.ExistingDocument<
									EntryDoc & PouchDB.Core.ChangesMeta
							  >
							| undefined;
						id: DocumentID;
					}[]
			...
			for (const chunk of nowSendChunks) {
				...
				if (
					(bulkDocsSizeBytes + jsonLength > maxBatchSizeBytes ||
						bulkDocsSizeCount + 1 > maxBatchSizeCount) &&
					bulkDocs.length > 0
				) {
	...
	async openOneShotReplication(
		...
		const next = await shareRunningResult("oneShotReplication", async () => {
			...
			const ret = await this.checkReplicationConnectivity(
				setting,
				false,
				retrying,
				showResult,
			...
			this.maxPushSeq = Number(
				`${(await localDB.info()).update_seq}`.split("-")[0],
			);
			if (showResult) {
				Logger(
					$msg("liveSyncReplicator.checkingLastSyncPoint"),
			...
			if (syncResult == "NEED_RETRY") {
				const tempSetting: RemoteDBSettings = JSON.parse(
					JSON.stringify(setting),
				);
				tempSetting.batch_size = Math.ceil(tempSetting.batch_size / 2) + 2;
				tempSetting.batches_limit =
					Math.ceil(tempSetting.batches_limit / 2) + 2;
	...
	async checkReplicationConnectivity(
		...
		const dbRet = await this.connectRemoteCouchDBWithSetting(
			setting,
			this.isMobile(),
			true,
		);
		...
		const next = await shareRunningResult("continuousReplication", async () => {
			...
			if (
				...
				if (syncResult == "NEED_RESURRECT") {
					this.terminateSync();
					return async () =>
						await this.openContinuousReplication(
							this.originalSetting,
							showResult,
							false,
						);
	...
	async tryResetRemoteDatabase(setting: RemoteDBSettings) {
		this.closeReplication();
		const con = await this.connectRemoteCouchDBWithSetting(
			setting,
			this.isMobile(),
			true,
		);
	...
	async tryCreateRemoteDatabase(setting: RemoteDBSettings) {
		this.closeReplication();
		const con2 = await this.connectRemoteCouchDBWithSetting(
			setting,
			this.isMobile(),
			true,
		);
		...
		await this.ensurePBKDF2Salt(setting, true, false);
		Logger(
			$msg("liveSyncReplicator.remoteDbCreatedOrConnected"),
			LOG_LEVEL_NOTICE,
		);
	}
	async markRemoteLocked(
		...
		const remoteMilestone: EntryMilestoneInfo = {
			...defInitPoint,
			...(await resolveWithIgnoreKnownError(
				dbRet.db.get(MILESTONE_DOCID),
				defInitPoint,
			)),
		};
		remoteMilestone.node_chunk_info = {
			...defInitPoint.node_chunk_info,
			...remoteMilestone.node_chunk_info,
		};
	...
	async markRemoteResolved(setting: RemoteDBSettings) {
		...
		remoteMilestone.accepted_nodes = Array.from(
			new Set([...remoteMilestone.accepted_nodes, this.nodeid]),
		);
		Logger($msg("liveSyncReplicator.markDeviceResolved"), LOG_LEVEL_NOTICE);
		const result = await dbRet.db.put(remoteMilestone);
		if (result.ok) {
			Logger(
				$msg("liveSyncReplicator.remoteDbMarkedResolved"),
				LOG_LEVEL_VERBOSE,
			);
		} else {
			Logger(
				$msg("liveSyncReplicator.couldNotMarkResolveRemoteDb"),
				LOG_LEVEL_NOTICE,
			);
		}
	}
	...
	async fetchRemoteChunks(
		...
		const remoteChunks = await ret.db.allDocs({
			keys: missingChunks,
			include_docs: true,
		});
		...
		const remoteChunkItems = remoteChunks.rows.map(
			(e: any) => e.doc as EntryLeaf,
		);
		return remoteChunkItems;
	}

	async tryConnectRemote(
		setting: RemoteDBSettings,
		showResult: boolean = true,
	): Promise<boolean> {
		const db = await this.connectRemoteCouchDBWithSetting(
			setting,
			this.isMobile(),
			true,
		);
	...
	async resetRemoteTweakSettings(setting: RemoteDBSettings): Promise<void> {
		...
		// check local database hash status and remote replicate hash status
		try {
			const remoteMilestone = (await dbRet.db.get(
				MILESTONE_DOCID,
			)) as EntryMilestoneInfo;
			remoteMilestone.tweak_values = {};
			await dbRet.db.put(remoteMilestone);
			Logger(
				`tweak values on the remote database have been cleared`,
				LOG_LEVEL_VERBOSE,
			);
		} catch (ex) {
	...
	async setPreferredRemoteTweakSettings(
		setting: RemoteDBSettings,
	): Promise<void> {
		const uri =
			setting.couchDB_URI +
		...
		try {
			...
			remoteMilestone.tweak_values[DEVICE_ID_PREFERRED] = extractObject(
				TweakValuesTemplate,
				{ ...setting },
			);
			await dbRet.db.put(remoteMilestone);
	...
	async getRemotePreferredTweakValues(
		setting: RemoteDBSettings,
	...
	async getConnectedDeviceList(
		...
		false | { node_info: Record<string, NodeData>; accepted_nodes: string[] }
	> {
		const dbRet = await this.connectRemoteCouchDBWithSetting(
			setting,
			this.isMobile(),
		...
		const acceptedNodes =
			(milestoneDoc as EntryMilestoneInfo).accepted_nodes || [];
