...
} from "../../pouchdb/LiveSyncDBFunctions.ts";
...
export class LiveSyncJournalReplicator extends LiveSyncAbstractReplicator {
	...
	get simpleStore() {
		return this.env.services.keyValueDB
			.simpleStore as SimpleStore<CheckPointInfo>;
	}
	...
	async ensureBucketIsCompatible(
		...
		const downloadedMilestone =
			await this.client.downloadJson<EntryMilestoneInfo>(MILSTONE_DOCID);
	...
	async openReplication(
		...
		if (!(await this.checkReplicationConnectivity(false, ignoreCleanLock)))
			return false;
	...
	async markRemoteLocked(
		...
		if (locked) {
			...
			Logger(
				"Unlock remote bucket to prevent data corruption",
				LOG_LEVEL_NOTICE,
			);
		}
	...
	async tryConnectRemote(
		...
		try {
			...
			Logger(
				`Error! Could not connected to ${endpoint}\n${(ex as Error).message}`,
				LOG_LEVEL_NOTICE,
			);
			Logger(ex, LOG_LEVEL_NOTICE);
			return false;
	...
	countCompromisedChunks(): Promise<number> {
		Logger(
			`Bucket Sync Replicator cannot count compromised chunks`,
			LOG_LEVEL_VERBOSE,
		);
