...
const RECORD_SPLIT = `\n`;
const UNIT_SPLIT = `\u001f`;
type ProcessingEntry = PouchDB.Core.PutDocument<EntryDoc> &
...
export abstract class JournalSyncAbstract {
	...
	async getSyncParameters(): Promise<SyncParameters> {
		try {
			const downloadedSyncParams = await this.downloadJson<SyncParameters>(
				DOCID_JOURNAL_SYNC_PARAMETERS,
			);
	...
	getHash(settings: BucketSyncSetting) {
		return btoa(
			encodeURI(
				[
					settings.endpoint,
					`${settings.bucket}${settings.bucketPrefix}`,
					settings.region,
				].join(),
			),
		);
	...
	constructor(
		...
		this.env = env;
		this.processReplication = async (docs) =>
	...
	async decryptDownloaded(
		...
		try {
			if (
				!set.encrypt ||
				set.passphrase == "" ||
				this.isEncryptionPrevented(key)
			) {
				return u;
			}
			...
			Logger(
				`Failed to decrypt in v2. Falling back to v1: ${key}`,
				LOG_LEVEL_INFO,
			);
			try {
	...
	async _createJournalPack(override?: number | string) {
		...
		const bd = await this.db.bulkGet({
			docs: allChanges.results
				.map((e) => e.changes.map((change) => ({ id: e.id, rev: change.rev })))
				.flat(),
			revs: true,
		});
	...
	async uploadQueued(showMessage = false, wrapUp = false) {
		return await shareRunningResult("upload_queue", async () => {
			...
			do {
				const queued =
					await this.trench.dequeuePermanentWithCommit<Uint8Array<ArrayBuffer>>(
						`upload_queue`,
					);
				...
				if (!queued) {
					...
					if (uploaded) {
						Logger(
							`${TASK_TITLE}: ${uploaded} files have been uploaded!`,
							logLevel,
							MSG_KEY,
						);
					} else {
						if (!wrapUp)
							Logger(`No files needs to be uploaded!`, logLevel, MSG_KEY);
					}
					return true;
				}
				const { key, value, commit, cancel, cancelCount, pendingItems } =
					queued;
				this.updateInfo({
					sent: uploaded,
					maxPushSeq: pendingItems + uploaded,
					lastSyncPushSeq: 1,
				});
				Logger(
				...
				if (cancelCount > 3) {
					Logger(
						`${TASK_TITLE} Something went wrong on processing queue ${key}.`,
						LOG_LEVEL_NOTICE,
					);
					return false;
				}
				...
				try {
					...
					Logger(
						`${TASK_TITLE}: Uploaded ${key} as ${filename}`,
						LOG_LEVEL_INFO,
					);
				} catch (ex) {
					Logger(
					...
					Logger(ex, LOG_LEVEL_VERBOSE);
					Logger(
						`${TASK_TITLE} Uploading ${key} cancelled for retry`,
						LOG_LEVEL_VERBOSE,
					);
					cancel();
		...
		return await shareRunningResult("create_send_data", async () => {
			try {
				...
				do {
					...
					const { changes, hasNext, packLastSeq } =
						await this._createJournalPack(currentLastSeq);
					const currentSeq = (packLastSeq as number) - startSeq;
					if (changes.length == 0) {
						isFinished = true;
					} else {
						Logger(
							`Packing Journal: ${currentSeq} / ${seqToProcess}`,
							logLevel,
							MSG_KEY,
						);
						...
						for (const row of changes) {
							...
							if (
								outBuf.length > maxOutBufLength ||
								binarySize > maxBinarySize
							) {
								const sendBuf = concatUInt8Array(outBuf);
								const orgLen = sendBuf.byteLength;
								const bin = await wrappedDeflate(sendBuf, {
									consume: true,
									level: 8,
								});
								Logger(
				...
				if (seqToProcess != 0) {
					Logger(
						`Packing Journal: Packaging ${seqToProcess}`,
						logLevel,
						MSG_KEY,
					);
				} else {
					Logger(
						`Packing Journal: No journals to be packed!`,
						logLevel,
						MSG_KEY,
					);
				}
	...
	async sendLocalJournal(showMessage = false) {
		...
		const results = await Promise.all([
			this.packAndCompress(showMessage),
			this.uploadQueued(showMessage),
		]);
	...
	async _getRemoteJournals() {
		...
		const files = (await this.listFiles(StartAfter)).filter(
			(e) => !e.startsWith("_"),
		);
		if (!files) return [];
	...
	async processDocuments(allDocs: ProcessingEntry[]) {
		...
		try {
			...
			try {
				...
				const ret = await this.db.bulkDocs<EntryDoc>(saveChunks, {
					new_edits: true,
				});
				...
				saveChunks
					...
					.forEach((doc) =>
						eventHub.emitEvent(REMOTE_CHUNK_FETCHED, doc as EntryLeaf),
					);
				await this.updateCheckPointInfo((info) => ({
					...info,
			...
			await this.processReplication(
				saveDocs as PouchDB.Core.ExistingDocument<EntryDoc>[],
			);
			await this.updateCheckPointInfo((info) => ({
				...info,
				knownIDs: setAllItems(
		...
		return await shareRunningResult("process_downloaded_journals", async () => {
			...
			do {
				...
				if (!queued) {
					...
					if (downloaded) {
						Logger(
							`${TASK_TITLE} ${downloaded} files have been uploaded!`,
							logLevel,
							MSG_KEY,
						);
					} else {
						if (!wrapUp)
							Logger(
								`${TASK_TITLE} No files needs to be processed!`,
								logLevel,
								MSG_KEY,
							);
					}
				...
				this.updateInfo({
					arrived: downloaded,
					maxPullSeq: pendingItems + downloaded,
					lastSyncPullSeq: 1,
				});
				Logger(
				...
				try {
					...
					Logger(
						`${TASK_TITLE}: Could not process downloaded journals`,
						LOG_LEVEL_NOTICE,
					);
					Logger(ex, LOG_LEVEL_VERBOSE);
					Logger(
						`${TASK_TITLE}: ${key} cancelled for retry`,
						LOG_LEVEL_VERBOSE,
					);
					cancel();
					await delay(1000); // Need Backoff?
	...
	async downloadRemoteJournals(showMessage = false) {
		return await shareRunningResult("downloadRemoteJournals", async () => {
			try {
				...
				const logLevel = showMessage ? LOG_LEVEL_NOTICE : LOG_LEVEL_INFO;
				Logger(
					"Receiving Journal: Getting list of remote journal",
					logLevel,
					"receivejournal",
				);
				...
				if (files.length == 0) {
					Logger(
						`Receiving Journal: No journals needs to be downloaded`,
						logLevel,
						"receivejournal",
					);
					return true;
				...
				for (const key of files) {
					count++;
					Logger(
						`Receiving Journal: ${count} / ${files.length}`,
						logLevel,
						"receivejournal",
					);
					if (this.requestedStop) {
					...
					if (this._currentCheckPointInfo.sentFiles.has(key)) {
						...
						Logger(
							`Receiving Journal: ${key} is own sent file`,
							LOG_LEVEL_VERBOSE,
						);
						await this.updateCheckPointInfo((info) => ({
							...info,
