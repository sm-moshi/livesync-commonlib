...
export class JournalSyncMinio extends JournalSyncAbstract {
	...
	_getClient(): S3 {
		...
		this._instance = new S3({
			...
			requestHandler: this.useCustomRequestHandler
				? this.env.services.API.getCustomFetchHandler()
				: undefined,
		...
		this._instance.middlewareStack.add(
			applyMd5BodyChecksumMiddleware({
				md5: Md5,
				base64Encoder: (data: Uint8Array) =>
					arrayBufferToBase64Sync(data.buffer),
	...
	async resetBucket() {
		...
		const r = await client.send(cmd);
		Logger(
			`${r?.Deleted?.length || 0} items has been deleted!`,
			LOG_LEVEL_NOTICE,
		);
	...
	async listFiles(from: string, limit?: number) {
		...
		return objects.Contents.filter((e) => e.Key?.startsWith(this.prefix)).map(
			(e) => e.Key?.substring(this.prefix.length),
		) as string[];
	}
	...
	async getUsage(): Promise<false | RemoteDBStatus> {
		...
		try {
			...
			if (!objects.Contents) return {};
			return {
				estimatedSize: objects.Contents.reduce(
					(acc, e) => acc + (e.Size || 0),
					0,
				),
			};
