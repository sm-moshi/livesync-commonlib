...
export class TrysteroReplicatorP2PServer {
	...
	async shutdown() {
		try {
			...
			Logger(
				`Some error has been occurred while shutting down the server`,
				LOG_LEVEL_INFO,
			);
			Logger(ex, LOG_LEVEL_VERBOSE);
		}
	...
	constructor(env: ReplicatorHostEnv, _serverPeerId = selfId) {
		...
		this.acceptedPeers = new StoredMapLike(
			this._env.simpleStore,
	...
	sendAdvertisement(peerId?: string) {
		...
		if (this._sendAdvertisement) {
			Logger(
				`peerId: ${this.serverPeerId} Sending Advertisement to ${peerId ?? "All"}`,
				LOG_LEVEL_VERBOSE,
			);
	...
	confirmUserToAccept(peerId: string) {
		return shareRunningResult(`confirmUserToAccept-${peerId}`, () =>
			this._confirmUserToAccept(peerId),
		);
	}
...
>[!INFO] You can revoke your decision from the Peer-to-Peer Replicator Pane.`;
		...
		const OPTIONS = [
			OPTION_ACCEPT,
			OPTION_IGNORE,
			OPTION_ACCEPT_TEMPORARILY,
			OPTION_IGNORE_TEMPORARILY,
		];
		return this.confirm
	...
	async isAcceptablePeer(peerId: string) {
		...
		if (this.temporaryAcceptedPeers.has(peerId))
			return this.temporaryAcceptedPeers.get(peerId);
		const accepted = await this.acceptedPeers.get(peerName);
		if (accepted !== undefined && accepted !== null) return accepted;
		const isAcceptable = (
			await this._acceptablePeers.update(this.settings)
		).value.some((e) => e.test(peerName));
		const isDeny = (
			await this._shouldDenyPeers.update(this.settings)
		).value.some((e) => e.test(peerName));
	...
	async start(bindings: BindableObject<any>[] = []) {
		...
		const passphrase =
			passphraseNumbers[0].toString(36) + passphraseNumbers[1].toString(36);
		await this.shutdown();
		if (!this.settings.P2P_Enabled) {
			Logger($msg("P2P.NotEnabled"), LOG_LEVEL_NOTICE);
			return;
		}
		const relays = this.settings.P2P_relays.split(",").filter(
			(e) => e.trim().length > 0,
		);
	...
	async __onRequest(data: Request, peerId: string) {
		try {
			...
			if (typeof func !== "function")
				throw new Error(
					`Cannot serve function ${data.type}, no function provided or I am only a client`,
				);
			...
			if (e instanceof ResponsePreventedError) {
				Logger(
					`Serving function: [FAILED] ${data.type}: Response prevented.`,
					LOG_LEVEL_VERBOSE,
				);
				return;
			}
			Logger(
				`Serving function: [FAILED] ${data.type} sending back the failure information`,
				LOG_LEVEL_VERBOSE,
			);
