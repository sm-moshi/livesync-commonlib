...
export class TrysteroReplicatorP2PClient {
			...
			this.invocationTimeouts.set(
				...
				setTimeout(() => {
					if (this.waitingInvocations.has(seq)) {
						...
						p.reject(
							new Error(
								`Invocation Timed out: ${type} (${seq}) (Timeout: ${timeout}ms)`,
							),
						);
					}
	...
	__onResponse(data: Response) {
		...
		if (data.error) {
			p.reject(data.error);
			Logger(
				`Invoking remote function [ DONE] : ${type} (${seq}) : (Error)`,
				LOG_LEVEL_VERBOSE,
			);
		} else {
			p.resolve(data.data);
	...
	bindRemoteFunction<T extends any[], U>(
		type: string,
	...
	async invokeRemoteFunction<T extends any[], U>(
		type: string,
		args: T,
		timeout = DEFAULT_RPC_TIMEOUT,
	) {
	...
	bindRemoteObjectFunctions<T extends BindableObject<any>, U extends keyof T>(
		key: U,
		timeout = DEFAULT_RPC_TIMEOUT,
	) {
		type F = T[U];
	...
	async invokeRemoteObjectFunction<
		T extends BindableObject<any>,
		U extends NonPrivateMethodKeys<T>,
	>(key: U, args: Parameters<T[U]>, timeout = DEFAULT_RPC_TIMEOUT) {
		if (!this.__send) {
			throw new Error("Not connected to any room");
		}
		return (await this._sendRPC(`${key.toString()}`, args, timeout)) as Awaited<
			ReturnType<T[U]>
		>;
	}
