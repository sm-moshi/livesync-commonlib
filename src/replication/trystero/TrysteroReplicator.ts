...
import {
	...
	serialized,
	skipIfDuplicated,
} from "octagonal-wheels/concurrency/lock_v2";
...
export class TrysteroReplicator {
	...
	constructor(env: ReplicatorHostEnv) {
		...
		try {
			...
			if (!this.settings.P2P_AppID) {
				throw new Error(
					"P2P App ID is not provided. We need it to establish the P2P connection",
				);
			}
			...
			if (!this.settings.P2P_relays || this.settings.P2P_relays.length === 0) {
				throw new Error(
					"No relay URIs provided. We need them to establish the P2P connection",
				);
			}
			...
			Logger(
				e instanceof Error
					? e.message
					: "Error while creating TrysteroReplicator",
				LOG_LEVEL_NOTICE,
			);
	...
	async getTweakSettings(fromPeerId: string) {
		...
		for (const key in allSettings) {
			...
			if (key == "passphrase") {
				...
				allSettings[key] = await getHashedStringWithCurrentTime(
					allSettings[key] ?? "",
				);
				continue;
			}
	...
	getCommands() {
		return {
			...
			getAllConfig: async (fromPeerId: string) => {
				...
				const passphrase = await skipIfDuplicated(
					`getAllConfig-${fromPeerId}`,
					async () => {
					...
					const r = JSON.stringify(
						Object.fromEntries(
							Object.entries(setting).map(([key, value]) => {
								return [
									key,
									"******".repeat(Math.ceil(Math.random() * 10) + 2),
								];
							}),
						),
					);
	...
	async requestAuthenticate(peerId: string) {
		...
		try {
			...
			const r = await connection.invokeRemoteObjectFunction<
				ReturnType<typeof this.getCommands>,
				"!reqAuth"
			>("!reqAuth", [selfPeerId], 20000);
			return r;
		} catch (e) {
	...
	async selectPeer() {
		...
		const selectedPeer = await this.confirm.askSelectString(
			"Select a peer to replicate",
			peers,
		);
	...
	async requestSynchroniseToPeer(
		...
		return await conn.invokeRemoteFunction(
			"reqSync",
	...
	async requestSynchroniseToAllAvailablePeers() {
		await scheduleOnceIfDuplicated(
			"requestSynchroniseToAllAvailablePeers",
			async () => {
				await delay(25);
				const replications = [...this.availableReplicationPairs].map(
					(peerId) => {
	...
	async sync(remotePeer: string, showNotice: boolean = false) {
		...
		Logger(
			`P2P Replication has been requested to ${remotePeer}`,
			logLevel,
			"p2p-replicator",
		);
	...
	async replicateTo() {
		...
		try {
			if (this._replicateToPeers.has(remotePeer)) {
				Logger(
					`Replication to ${remotePeer} is already in progress`,
					LOG_LEVEL_VERBOSE,
				);
				return;
			}
	...
	acknowledgeProgress(remotePeerId: string, info?: ProgressInfo) {
		...
		try {
			void connection.invokeRemoteFunction(
				"onProgressAcknowledged",
				[this.server.serverPeerId, info],
				500,
			);
		} catch (ex) {
			Logger("Error while acknowledging the progress", LOG_LEVEL_VERBOSE);
	...
	async replicateFrom(
		...
		const logLevel = showNotice ? LOG_LEVEL_NOTICE : LOG_LEVEL_INFO;
		Logger(
			`P2P Requesting Authentication to ${remotePeer}`,
			logLevel,
			"p2p-replicator",
		);
		...
		if (this._replicateFromPeers.has(remotePeer)) {
			Logger(
				`Replication from ${remotePeer} is already in progress`,
				LOG_LEVEL_NOTICE,
				"p2p-replicator",
			);
			return {
				error: new Error("Replication from this peer is already in progress"),
			};
		}
		...
		try {
			...
			await replicateShim(
				...
				async (docs, info) => {
					await this._env.processReplicatedDocs(
						docs as Array<PouchDB.Core.ExistingDocument<EntryDoc>>,
					);
			...
			Logger(
				`P2P Replication from ${remotePeer} has been completed`,
				logLevel,
				"p2p-replicator",
			);
		} catch (e) {
			...
			void serialized(`notifyProgress-${peerId}`, async () => {
				...
				if (isAcceptable) {
					try {
						...
						Logger(
							`Error while notifying progress to ${peerId}`,
							LOG_LEVEL_VERBOSE,
						);
						Logger(e, LOG_LEVEL_VERBOSE);
					}
					...
					Logger(
						`Peer ${peerId} is not acceptable to notify progress`,
						LOG_LEVEL_VERBOSE,
					);
				}
			});
	...
	async getRemoteIsBroadcasting(peerId: string) {
		try {
			return await this.server
				?.getConnection(peerId)
				.invokeRemoteFunction("getIsBroadcasting", [], 0);
		} catch (e) {
	...
	async getRemoteConfig(peerId: string) {
		...
		const connection = this.server.getConnection(peerId);
		const encryptedConfig = await connection.invokeRemoteFunction(
			"getAllConfig",
			[this.server.serverPeerId],
			0,
		);
	...
	async checkTweakValues(peerId: string) {
		...
		const peerPlatform = this.server.knownAdvertisements.find(
			(e) => e.peerId == peerId,
		)?.platform;
	...
	async replicateFromCommand(showResult: boolean = false) {
		const r = await skipIfDuplicated("replicateFromCommand", async () => {
			...
			for (const peer of peers) {
				const peerId = this.knownAdvertisements.find(
					(e) => e.name == peer,
				)?.peerId;
				if (!peerId) {
