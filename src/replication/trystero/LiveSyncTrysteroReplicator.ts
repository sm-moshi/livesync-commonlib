...
let replicatorInstanceGetter: () => TrysteroReplicator | undefined = () =>
	undefined;
...
export class LiveSyncTrysteroReplicator extends LiveSyncAbstractReplicator {
	...
	async selectPeer(
		settingPeerName: string,
		r: TrysteroReplicator,
		logLevel: LOG_LEVEL,
		...
		if (knownPeersOrg.length != 0) {
			...
			await Promise.race([
				delay(5000),
				eventHub.waitFor(EVENT_ADVERTISEMENT_RECEIVED),
			]);
		...
		const markedPeerNames = knownPeers.map(
			(e) =>
				`${e.name}\u2001${e.name == settingPeerName ? `[*]` : ""} (${e.peerId})`,
		);
	...
	async tryUntilSuccess<T>(
		func: () => Promise<T | false>,
		repeat: number,
		logLevel: LOG_LEVEL,
	): Promise<T | false> {
		const confirm = this.env.services.UI.confirm;
	...
	async replicateAllFromServer(
		...
		if (setting.P2P_Enabled == false) {
			...
			const confirm = this.env.services.UI.confirm;
			if (
				(await confirm.askYesNoDialog($msg("P2P.DisabledButNeed"), {})) != "yes"
			) {
