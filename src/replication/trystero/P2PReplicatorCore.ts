...
import {
	reactiveSource,
	type ReactiveSource,
} from "octagonal-wheels/dataobject/reactive";
import {
...
export function addP2PEventHandlers(instance: CommandShim) {
	eventHub.onEvent(EVENT_ADVERTISEMENT_RECEIVED, (peerId) =>
		instance._replicatorInstance?.onNewPeer(peerId),
	);
	eventHub.onEvent(EVENT_DEVICE_LEAVED, (info) =>
		instance._replicatorInstance?.onPeerLeaved(info),
	);
...
export class P2PLogCollector {
	...
	updateP2PReplicationLine() {
		const p2pReplicationResultX = [...this.p2pReplicationResult.values()].sort(
			(a, b) => a.peerId.localeCompare(b.peerId),
		);
