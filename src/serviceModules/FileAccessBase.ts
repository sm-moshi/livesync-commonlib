...
export function toArrayBuffer(
	arr: Uint8Array<ArrayBuffer> | ArrayBuffer | DataView<ArrayBuffer>,
): ArrayBuffer {
...
export interface AbstractFolder<TItems extends AbstractInfo = AbstractInfo>
...
export abstract class FileAccessBase<
	...
	TNativeFile extends TNativeAbstractFile & AbstractFile = TNativeAbstractFile &
		AbstractFile,
	TNativeFolder extends TNativeAbstractFile &
		AbstractFolder<TNativeAbstractFile> = TNativeAbstractFile &
...
> {
	...
	protected abstract markChangesAreSame(
		path: string,
		mtime: number,
		newMtime: number,
	): void;
	...
	protected abstract _trash(
		file: TNativeAbstractFile,
		force?: boolean,
	): Promise<void>;
	protected abstract _getAbstractFileByPathInsensitive(
		path: FilePath | string,
	): TNativeAbstractFile | null;
	protected abstract _getAbstractFileByPath(
		path: FilePath | string,
	): TNativeAbstractFile | null;
	...
	protected abstract _vaultModify(
		file: TNativeFile,
		data: string,
	...
	protected abstract _vaultCreate(
		path: string,
		data: string,
		options?: UXDataWriteOptions,
	): Promise<TNativeFile>;
	...
	protected abstract _statFromNative(
		file: TNativeFile,
	): Promise<TNativeFile["stat"]>;
	...
	protected abstract _adapterWrite(
		file: string,
		data: string,
		options?: UXDataWriteOptions,
	): Promise<void>;
	...
	protected abstract _adapterExists(
	...
	protected abstract _adapterList(
		basePath: string,
	...
	protected _writeOp<T extends TNativeAbstractFile | string, U>(
		...
		return this.storageAccessManager.processWriteFile(
			path,
			async () => await callback(path, file),
		);
	}
	...
	protected _readOp<T extends TNativeAbstractFile | string, U>(
		...
		const path = this.getPath(file);
		return this.storageAccessManager.processReadFile(
			path,
			async () => await callback(path, file),
		);
	}
	...
	async tryAdapterStat(file: TNativeFile | string) {
		return await this._readOp(
			file,
			async (path) => await this._tryAdapterStat(path),
		);
	}
	...
	async adapterStat(file: TNativeFile | string): Promise<TStat | null> {
		return await this._readOp(
			file,
			async (path) => await this._adapterStat(path),
		);
	}
	...
	async adapterExists(file: TNativeFile | string): Promise<boolean> {
		return await this._readOp(
			file,
			async (path) => await this._adapterExists(path),
		);
	}
	...
	async adapterRemove(file: TNativeFile | string): Promise<void> {
		return await this._writeOp(
			file,
			async (path) => await this._adapterRemove(path),
		);
	}
	...
	async adapterRead(file: TNativeFile | string): Promise<string> {
		return await this._readOp(
			file,
			async (path) => await this._adapterRead(path),
		);
	}
	...
	async adapterReadBinary(file: TNativeFile | string): Promise<ArrayBuffer> {
		return await this._readOp(
			file,
			async (path) => await this._adapterReadBinary(path),
		);
	}

	async adapterReadAuto(
		file: TNativeFile | string,
	...
	async adapterWrite(
		...
		if (typeof data === "string") {
			return await this._writeOp(
				file,
				async (path) => await this._adapterWrite(path, data, options),
			);
		} else {
			return await this._writeOp(
				file,
				async (path) =>
					await this._adapterWriteBinary(path, toArrayBuffer(data), options),
			);
		}
	}
	...
	async vaultCacheRead(file: TNativeFile): Promise<string> {
		return await this._readOp(
			file,
			async (path) => await this._vaultCacheRead(file),
		);
	}
	...
	vaultReadBinary(file: TNativeFile): Promise<ArrayBuffer> {
		return this._readOp(
			file,
			async (path) => await this._vaultReadBinary(file),
		);
	}
	...
	async vaultReadAuto(file: TNativeFile) {
		...
		if (isPlainText(path)) {
			return await this._readOp(
				path,
				async (path) => await this._vaultRead(file),
			);
		}
		return await this._readOp(
			path,
	...
	async vaultModify(
		...
		if (typeof data === "string") {
			return await this._writeOp(file, async (path) => {
				...
				if (data === oldData) {
					...
					if (options && options.mtime)
						this.markChangesAreSame(path, stat.mtime, options.mtime);
					return true;
				}
			...
			return await this._writeOp(file, async (path) => {
				...
				if (
					await isDocContentSame(
						createBinaryBlob(oldData),
						createBinaryBlob(data),
					)
				) {
					const stat = await this._statFromNative(file);
					if (options && options.mtime)
	...
	async vaultCreate(
		...
		if (typeof data === "string") {
			return await this._writeOp(path as FilePath, () =>
				this._vaultCreate(path, data, options),
			);
		} else {
			return await this._writeOp(path as FilePath, () =>
	...
	async delete(file: TNativeAbstractFile, force?: boolean): Promise<void> {
		return await this._writeOp(
			file,
			async (path, file) => await this._delete(file, force),
		);
	}
	...
	async trash(file: TNativeAbstractFile, force?: boolean): Promise<void> {
		return await this._writeOp(
			file,
			async (path, file) => await this._trash(file, force),
		);
	}
	...
	async touch(file: TNativeFile | FilePath): Promise<void> {
		const path = this.getPath(file);
		const statOrg = this.isFile(file)
			? file.stat
			: await this._adapterStat(path);
		return this.storageAccessManager.touch({
			path,
			stat: statOrg || { ctime: 0, mtime: 0, size: 0 },
		});
	}

	recentlyTouched(file: TNativeFile | UXFileInfoStub | FileWithFileStat) {
		return this.storageAccessManager.recentlyTouched({
			...file,
