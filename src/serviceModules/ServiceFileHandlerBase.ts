...
import {
	...
	type BASE_IS_NEW,
	EVEN,
} from "@lib/common/models/shared.const.symbols";
	...
	implements IFileHandler
{
	...
	constructor(services: ServiceFileHandlerDependencies) {
		...
		services.fileProcessing.processFileEvent.addHandler(
			this._anyHandlerProcessesFileEvent.bind(this),
		);
		services.replication.processSynchroniseResult.addHandler(
			this._anyProcessReplicatedDoc.bind(this),
		);
	}
	get db() {
	...
	async storeFileToDB(
		...
		const file =
			typeof info === "string" ? this.storage.getFileStub(info) : info;
		if (file == null) {
		...
		if (!force && !onlyChunks) {
			...
			if (!shouldApplied) {
				...
				if (!readFile) {
					this._log(
						`File ${file.path} is not exist on the storage`,
						LOG_LEVEL_NOTICE,
					);
					return false;
				}
				if (
					await isDocContentSame(getDocDataAsArray(entry.data), readFile.body)
				) {
	...
	async deleteFileFromDB(
		info: UXFileInfoStub | UXInternalFileInfoStub | FilePath,
	): Promise<boolean> {
		const file =
			typeof info === "string" ? this.storage.getFileStub(info) : info;
		if (file == null) {
		...
		if (!entry || entry.deleted || entry._deleted) {
			this._log(
				`File ${file.path} is not exist or already deleted on the database`,
				LOG_LEVEL_VERBOSE,
			);
			return false;
		}
	...
	async resolveConflictedByDeletingRevision(
		...
		if (!(await this.deleteRevisionFromDB(info, rev))) {
			this._log(
				`Failed to delete the conflicted revision ${rev} of ${path}`,
				LOG_LEVEL_VERBOSE,
			);
			return false;
		}
		if (!(await this.dbToStorageWithSpecificRev(info, rev, true))) {
			this._log(
				`Failed to apply the resolved revision ${rev} of ${path} to the storage`,
				LOG_LEVEL_VERBOSE,
			);
			return false;
		}
	...
	async dbToStorageWithSpecificRev(
		...
		if (!docEntry) {
			this._log(
				`File ${file.path} is not exist on the database`,
				LOG_LEVEL_VERBOSE,
			);
			return false;
		}
	...
	async dbToStorage(
		...
		const pathFromEntryInfo =
			typeof entryInfo === "string" ? entryInfo : this.getPath(entryInfo);
		const docEntry = await this.db.fetchEntryMeta(
			pathFromEntryInfo,
			undefined,
			true,
		);
		if (!docEntry) {
			this._log(
				`File ${pathFromEntryInfo} is not exist on the database`,
				LOG_LEVEL_VERBOSE,
			);
			return false;
		}
		...
		if (existDoc && existDoc.isFolder) {
			this._log(
				`Folder ${path} is already exist on the storage as a folder`,
				LOG_LEVEL_VERBOSE,
			);
		...
		if (!existOnDB && !existOnStorage) {
			this._log(
				`File ${path} seems to be deleted, but already not on storage`,
				LOG_LEVEL_VERBOSE,
			);
			return true;
		}
		...
		const ret = await this.storage.writeFileAuto(path, docData, {
			ctime: docRead.ctime,
			mtime: docRead.mtime,
		});
	...
	private async _anyHandlerProcessesFileEvent(
		item: FileEventItem,
	): Promise<boolean> {
	...
	async _anyProcessReplicatedDoc(entry: MetaEntry): Promise<boolean> {
		return await serialized(entry.path, async () => {
			if (!(await this.vault.isTargetFile(entry.path))) {
				this._log(
					`File ${entry.path} is not the target file`,
					LOG_LEVEL_VERBOSE,
				);
				return false;
			}
			if (this.vault.isFileSizeTooLarge(entry.size)) {
				this._log(
					`File ${entry.path} is too large (on database) to be processed`,
					LOG_LEVEL_VERBOSE,
				);
				return false;
			}
			...
			if (targetFile && targetFile.isFolder) {
				...
				if (targetFile && this.vault.isFileSizeTooLarge(targetFile.stat.size)) {
					this._log(
						`File ${targetFile.path} is too large (on storage) to be processed`,
						LOG_LEVEL_VERBOSE,
					);
					return false;
				}
				...
				this._log(
					`Processing ${path} (${entry._id.substring(0, 8)} :${entry._rev?.substring(0, 5)}) : Done`,
				);
				return ret;
			}
		});
