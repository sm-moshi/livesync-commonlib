...
import {
	...
	isPlainText,
	stripAllPrefixes,
} from "@lib/string_and_binary/path";
	...
	implements DatabaseFileAccess
{
	...
	private async __store(
		...
		const isNotChanged = await serialized("file-" + fullPath, async () => {
			...
			// }
			try {
				const old = await this.database.localDatabase.getDBEntry(
					d.path,
					undefined,
					false,
					true,
				...
				if (old !== false) {
					const oldData = {
						data: old.data,
						deleted: old._deleted || old.deleted,
					};
	...
	async fetchEntryFromMeta(
		...
		const doc = await this.database.localDatabase.getDBEntryFromMeta(
			meta as LoadedEntry,
			false,
			waitForReady,
		);
	...
	async deleteFromDBbyPath(
		fullPath: FilePath | FilePathWithPrefix,
		rev?: string,
	): Promise<boolean> {
