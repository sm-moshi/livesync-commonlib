...
export class ServiceRebuilder
	extends ServiceModuleBase<ServiceRebuilderDependencies>
	implements Rebuilder
{
	...
	constructor(services: ServiceRebuilderDependencies) {
		...
		services.database.onDatabaseReset.addHandler(
			this._onResetLocalDatabase.bind(this),
		);
		...
		services.setting.suspendAllSync.addHandler(
			this._allSuspendAllSync.bind(this),
		);
	}
...
Please enable them from the settings screen after setup is complete.`,
	...
	async rebuildRemote() {
		...
		await this.setting.applyPartial({
			...
			notifyThresholdOfRemoteStorageSize:
				DEFAULT_SETTINGS.notifyThresholdOfRemoteStorageSize,
		});
	...
	$fetchLocal(
		...
		return this.fetchLocal(
			makeLocalChunkBeforeSync,
			preventMakeLocalFilesBeforeSync,
		);
	}

	async scheduleRebuild(): Promise<void> {
		try {
			await this.storageAccess.writeFileAuto(
				FlagFilesHumanReadable.REBUILD_ALL,
				"",
			);
		} catch (ex) {
			this._log(
				`Could not create ${FlagFilesHumanReadable.REBUILD_ALL}`,
				LOG_LEVEL_NOTICE,
			);
	...
	async scheduleFetch(): Promise<void> {
		try {
			await this.storageAccess.writeFileAuto(
				FlagFilesHumanReadable.FETCH_ALL,
				"",
			);
		} catch (ex) {
			this._log(
				`Could not create ${FlagFilesHumanReadable.FETCH_ALL}`,
				LOG_LEVEL_NOTICE,
			);
			this._log(ex, LOG_LEVEL_VERBOSE);
		}
...
Are you sure you wish to proceed?`;
	...
	async resetLocalDatabase() {
		...
		if (
			...
			settings.additionalSuffixOfDatabaseName == ""
		) {
