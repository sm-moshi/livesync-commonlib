...
import type {
	...
	StorageAccess,
} from "@lib/interfaces/StorageAccess.ts";
...
import type {
	...
	AbstractInfo,
	FileAccessBase,
} from "@lib/serviceModules/FileAccessBase";
...
export class ServiceFileAccessBase<
		TNativeAbstractFile extends AbstractInfo = AbstractInfo,
		TNativeFile extends TNativeAbstractFile &
			AbstractFile = TNativeAbstractFile & AbstractFile,
		TNativeFolder extends TNativeAbstractFile &
			AbstractFolder<TNativeAbstractFile> = TNativeAbstractFile &
			AbstractFolder<TNativeAbstractFile>,
		TStat extends UXStat = UXStat,
	>
	extends ServiceModuleBase<
		StorageAccessBaseDependencies<
			TNativeAbstractFile,
			TNativeFile,
			TNativeFolder,
			TStat
...
{
	private vaultAccess: FileAccessBase<
		TNativeAbstractFile,
		TNativeFile,
		TNativeFolder,
		TStat
	...
	constructor(
		services: StorageAccessBaseDependencies<
			TNativeAbstractFile,
			TNativeFile,
			TNativeFolder,
			TStat
		...
		services.appLifecycle.onFirstInitialise.addHandler(
			this._everyOnFirstInitialize.bind(this),
		);
		services.fileProcessing.commitPendingFileEvents.addHandler(
			this._everyCommitPendingFileEvent.bind(this),
		);
	}
	...
	async writeFileAuto(
		...
		if (this.vaultAccess.isFile(file)) {
			...
			this._log(
				`Could not write file (Possibly already exists as a folder): ${path}`,
				LOG_LEVEL_VERBOSE,
			);
			return false;
		}
	...
	isExists(path: string): Promise<boolean> {
		return Promise.resolve(
			this.vaultAccess.isFile(this.vaultAccess.getAbstractFileByPath(path)),
		);
	}
	async writeHiddenFileAuto(
		path: string,
		data: string | ArrayBuffer,
		opt?: UXDataWriteOptions,
	): Promise<boolean> {
	...
	async readStubContent(stub: UXFileInfoStub): Promise<UXFileInfo | false> {
		...
		if (!this.vaultAccess.isFile(file)) {
			this._log(
				`Could not read file (Possibly does not exist or a folder): ${stub.path}`,
				LOG_LEVEL_VERBOSE,
			);
			return false;
		}
	...
	getFiles(): UXFileInfoStub[] {
		return this.vaultAccess
			.getFiles()
			.map((f) => this.vaultAccess.nativeFileToUXFileInfoStub(f));
	}
	...
	async getFilesIncludeHidden(
		...
		try {
			...
			this._log(
				`Could not traverse(getFilesIncludeHidden):${basePath}`,
				LOG_LEVEL_INFO,
			);
		...
		for (const v of w.folders) {
			...
			// OK, deep dive!
			files = files.concat(
				await this.getFilesIncludeHidden(
					v,
					includeFilter,
					excludeFilter,
					skipFolder,
	...
	recentlyTouched(file: UXFileInfoStub | FilePathWithPrefix): boolean {
		const xFile =
			typeof file === "string"
				? (this.vaultAccess.getAbstractFileByPath(file) as TNativeFile)
				: file;
	...
	delete(
		file: FilePathWithPrefix | UXFileInfoStub | string,
		force: boolean,
	): Promise<void> {
		...
		if (!this.vaultAccess.isFile(xFile) && !this.vaultAccess.isFolder(xFile))
			return Promise.resolve();
