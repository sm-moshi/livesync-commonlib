import {
	...
	type AppLifecycleServiceDependencies,
} from "@lib/services/base/AppLifecycleService";
...
} from "./implements/headless/HeadlessDatabaseService";
import {
...
class HeadlessAppLifecycleService<
...
class HeadlessSvelteDialogManager<
	T extends ServiceContext,
> extends SvelteDialogManagerBase<T> {
	openSvelteDialog<T, U>(
...
class HeadlessUIService extends UIService<ServiceContext> {
	...
	constructor(
		...
		dependents: HeadlessUIServiceDependencies<ServiceContext>,
	) {
		const headlessConfirm = dependents.APIService.confirm;
		const headlessSvelteDialogManager =
			new HeadlessSvelteDialogManager<ServiceContext>(context, {
