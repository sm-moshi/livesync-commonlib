...
type TargetFunc<TArgs extends any[], TResult> = (
	...args: TArgs
) => Promise<TResult>;
...
export class MiddlewareManager<TArgs extends any[], TResult> {
	private middlewares: {
		func: MiddlewareFunc<TArgs, TResult>;
		priority: number;
		index: number;
	}[] = [];
	...
	private composed: TargetFunc<TArgs, TResult> = () => {
		throw new Error(
			"MiddlewareManager: compose must be called before using the composed function.",
		);
	};
	...
	return {
		useMiddleware<K extends FunctionKeys<T>>(key: K) {
			...
			return {
				use: (
					priority: number,
					func: MiddlewareFunc<Parameters<T[K]>, Awaited<ReturnType<T[K]>>>,
				) => {
				...
				setFinal: (
					func: TargetFunc<Parameters<T[K]>, Awaited<ReturnType<T[K]>>>,
				) => {
