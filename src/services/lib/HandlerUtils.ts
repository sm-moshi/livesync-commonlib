 ...
 */
type HandlerFunc<TArg extends any[], TResult> = (
	...args: TArg
 ...
 */
type BooleanHandlerFunc<TArg extends any[], U = boolean> = (
	...args: TArg
) => U | Promise<U>;
 ...
 */
export interface DispatchHandler<T extends any[], U>
	extends DispatcherHandler<T, U>,
		MultiRegisterHandler<T, U> {}
/**
...
export class Binder<T extends HandlerFunc<any, any>>
	implements
		...
		InvokableHandler<Parameters<T>, ReturnType<T>>
{
 ...
 * */
export class Dispatch<T extends any[], U>
	...
	async invoke(...args: T): Promise<boolean> {
		...
		for (const callback of _callbacks) {
			try {
				...
				Logger(
					`AllHandler ${this._name} treated error as failure: ${error}`,
					LOG_LEVEL_VERBOSE,
				);
	...
	async invoke(...args: T): Promise<boolean> {
		...
		const promises: Promise<boolean>[] = callbacks.map(async (callback) => {
			try {
				...
				// On error, consider it as failure
				Logger(
					`ParallelAllHandler ${this._name} treated error as failure: ${error}`,
					LOG_LEVEL_VERBOSE,
				);
				return false;
			}
	...
	async invoke(...args: T): Promise<boolean> {
		...
		for (const callback of _callbacks) {
			try {
				...
				Logger(
					`FirstSuccessHandler ${this._name} ignored error: ${error}`,
					LOG_LEVEL_VERBOSE,
				);
			}
		}
 ...
 */
export class FirstResultHandler<T extends any[], U> extends MultiBinder<
	BooleanHandlerFunc<T, U>
> {
	/**
	...
	async invoke(...args: T): Promise<U | false> {
		...
		for (const callback of _callbacks) {
			try {
				...
				Logger(
					`FirstResultHandler ${this._name} ignored error: ${error}`,
					LOG_LEVEL_VERBOSE,
				);
			}
		}
...
export interface HandlerFunction<
	TFunc extends (...args: any[]) => any | Promise<any>,
> {
	/**
 ...
 */
export interface MultipleHandlerFunction<
	TFunc extends (...args: any[]) => any | Promise<any>,
> {
	/**
	 * Invokes the handler function with the provided arguments.
...
export interface BooleanMultipleHandlerFunction<
...
export interface MultiBinderInstance<T extends any[], U>
	extends InvokableHandler<T, U>,
		MultiRegisterHandler<T, U> {}
...
function getMultipleBound<T extends MultiBinderInstance<any, any>>(
	handler: T,
): MultipleHandlerFunction<T["invoke"]>;
function getMultipleBound<T extends DispatchHandler<any, any>>(
	handler: T,
): CollectiveHandlerFunction<T["dispatch"]>;
function getMultipleBound<
	T extends MultiBinderInstance<any, any> | DispatchHandler<any, any>,
>(handler: T) {
	const _handler = "invoke" in handler ? handler.invoke : handler.dispatch;
	const __handler = _handler.bind(handler);
	const func = (
		...args: Parameters<typeof __handler>
	): ReturnType<typeof __handler> => {
...
export function bailFirstFailureFunction<
	TFunc extends (...args: any[]) => Promise<boolean>,
>(name?: string): BooleanMultipleHandlerFunction<TFunc> {
	const handler = new AllHandler<Parameters<TFunc>>(
		name ?? "bailFirstFailureFunction",
	);
	return getMultipleBound(handler);
}

export function allParallelFunction<
	TFunc extends (...args: any[]) => Promise<boolean>,
>(name?: string): BooleanMultipleHandlerFunction<TFunc> {
	const handler = new ParallelAllHandler<Parameters<TFunc>>(
		name ?? "allParallelFunction",
	);
	return getMultipleBound(handler);
}

export function anySuccessFunction<
	TFunc extends (...args: any[]) => Promise<boolean>,
>(name?: string): BooleanMultipleHandlerFunction<TFunc> {
	const handler = new AnySuccessHandler<Parameters<TFunc>>(
		name ?? "anySuccessFunction",
	);
	return getMultipleBound(handler);
}

export function firstResultFunction<
	TFunc extends (...args: any[]) => Promise<any>,
>(name?: string): MultipleHandlerFunction<TFunc> {
	const handler = new FirstResultHandler<Parameters<TFunc>, ReturnType<TFunc>>(
		name ?? "firstResultFunction",
	);
	return getMultipleBound(handler);
}

export function dispatchParallelFunction<
	TFunc extends (...args: any[]) => Promise<any[]>,
>(name?: string): CollectiveHandlerFunction<TFunc> {
	const handler = new DispatchParallel<
		Parameters<TFunc>,
		Awaited<ReturnType<TFunc>>[number]
	>(name ?? "dispatchParallelFunction");
	return getMultipleBound(handler);
}
export function bindableFunction<TFunc extends (...args: any[]) => any>(
	name?: string,
): HandlerFunction<TFunc> {
...
export function lazyBindableFunction<TFunc extends (...args: any[]) => any>(
	name?: string,
): LazyHandlerFunction<TFunc> {
...
export function handlers<
	T extends Record<keyof T, ((...args: any[]) => any) | any>,
>() {
	return {
		/**
		...
		all<K extends FunctionKeys<T>>(
			name: K,
		): BooleanMultipleHandlerFunction<T[K]> {
