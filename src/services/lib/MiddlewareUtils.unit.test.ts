...
describe("MiddlewareManager", () => {
	it("should execute middlewares in order and return final result", async () => {
		...
		x.use(10, async (ctx, file) => {
			console.log(
				"Middleware 1: Checking if file is target by duplication: " + file,
			);
		...
		x.use(20, async (ctx, file) => {
			console.log(
				"Middleware 2: Checking if file is target by local database: " + file,
			);
			const result = await ctx.next();
		...
		x.setFinal(async (file) => {
			console.log(
				"Final function: Checking if file is target by default logic: " + file,
			);
	...
	describe("should allow middlewares to short-circuit the chain, respects priority", async () => {
		...
		x.use(20, async (ctx, file) => {
			...
			// const result = await ctx.next();
			console.log(
				"Middleware 2: Short-circuiting the chain and returning true, indicating it's a target file: " +
					file,
			);
