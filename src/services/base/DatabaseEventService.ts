 ...
 */
export abstract class DatabaseEventService<
	...
	implements IDatabaseEventService
{
	 ...
	 */
	readonly onUnloadDatabase =
		handlers<IDatabaseEventService>().all("onUnloadDatabase");

	/**
	 * Event triggered when the database is about to be closed.
	 */
	readonly onCloseDatabase =
		handlers<IDatabaseEventService>().all("onCloseDatabase");

	/**
	 * Event triggered when the database is being initialized.
	 */
	readonly onDatabaseInitialisation =
		handlers<IDatabaseEventService>().bailFirstFailure(
	 ...
	 */
	readonly onDatabaseInitialised =
		handlers<IDatabaseEventService>().bailFirstFailure("onDatabaseInitialised");

	/**
	 * Event triggered when the database is being reset.
	 */
	readonly onResetDatabase =
		handlers<IDatabaseEventService>().bailFirstFailure("onResetDatabase");

	/**
	 * Event triggered when the database is ready for use.
	 */
	readonly onDatabaseHasReady =
		handlers<IDatabaseEventService>().bailFirstFailure("onDatabaseHasReady");
	/**
