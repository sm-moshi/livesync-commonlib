...
import {
	createInstanceLogFunction,
	MARK_LOG_NETWORK_ERROR,
	type LogFunction,
} from "../lib/logUtils";
	...
	implements IReplicationService
{
	 ...
	 */
	readonly processSynchroniseResult =
		handlers<IReplicationService>().anySuccess("processSynchroniseResult");

	/**
	 * Process a synchronisation result document for optional entries i.e., hidden files.
	 */
	readonly processOptionalSynchroniseResult =
		handlers<IReplicationService>().anySuccess(
	 ...
	 */
	readonly parseSynchroniseResult = handlers<IReplicationService>().all(
		"parseSynchroniseResult",
	);
	/**
	 * Process a virtual document (e.g., for customisation sync).
	 */
	readonly processVirtualDocument = handlers<IReplicationService>().anySuccess(
	 ...
	 */
	readonly onBeforeReplicate =
		handlers<IReplicationService>().bailFirstFailure("onBeforeReplicate");

	/**
	 *
	 */
	readonly onCheckReplicationReady =
		handlers<IReplicationService>().bailFirstFailure("onCheckReplicationReady");
	...
	async isReplicationReady(showMessage: boolean = false): Promise<boolean> {
		...
		if (!(await this.onBeforeReplicate(showMessage))) {
			...
			const hasNetworkError = (
				await this.appLifecycleService.getUnresolvedMessages()
			)
				.flat()
				.some(
					(e) =>
						typeof e == "string" && e.indexOf(MARK_LOG_NETWORK_ERROR) !== -1,
				);
			if (!hasNetworkError) {
				this.showError(
					$msg("Replicator.Message.SomeModuleFailed"),
					LOG_LEVEL_NOTICE,
				);
			} else {
	...
	onReplicationFailed = handlers<IReplicationService>().bailFirstFailure(
		"onReplicationFailed",
	);
	...
	async performReplication(showMessage?: boolean): Promise<boolean | void> {
		...
		if (!activeReplicator) {
			this._log(
				`No active replicator found`,
				showMessage ? LOG_LEVEL_NOTICE : LOG_LEVEL_INFO,
			);
			return false;
		}
		const settings = this.settingService.currentSettings();
		const ret = await activeReplicator.openReplication(
			settings,
			false,
			!!showMessage,
			false,
	 ...
	 */
	readonly checkConnectionFailure = handlers<IReplicationService>().firstResult(
	...
	getActiveReplicatorFor(usage: string) {
		...
		if (!activeReplicator) {
			this._log(
				`Active replicator not found during ${usage}`,
				LOG_LEVEL_NOTICE,
			);
			return false;
		}
	...
	async replicateAllToRemote(
		...
		const currentSettings = this.settingService.currentSettings();
		const activeReplicator = this.getActiveReplicatorFor(
		...
		if (!sendChunksInBulkDisabled) {
			if (activeReplicator?.isChunkSendingSupported) {
				if (
					...
					await activeReplicator.sendChunks(
						currentSettings,
						undefined,
						true,
						0,
		...
		const ret = await activeReplicator.replicateAllToServer(
			currentSettings,
			showingNotice,
		);
		if (ret) return true;
		...
		if (checkResult == "CHECKAGAIN")
			return await activeReplicator.replicateAllToServer(
				currentSettings,
				showingNotice,
			);
		return !checkResult;
	...
	async replicateAllFromRemote(
		...
		const currentSettings = this.settingService.currentSettings();
		const ret = await activeReplicator.replicateAllFromServer(
			currentSettings,
			showingNotice,
		);
		if (ret) return true;
		const checkResult = await this.checkConnectionFailure();
		if (checkResult == "CHECKAGAIN")
			return await activeReplicator.replicateAllFromServer(
				currentSettings,
				showingNotice,
			);
		return !checkResult;
	...
	async markLocked(lockByClean: boolean = false): Promise<void> {
		return await this._getReplicatorAndPerform(
			...
			async (currentSettings, activeReplicator) => {
				return await activeReplicator.markRemoteLocked(
					currentSettings,
					true,
					lockByClean,
				);
