 ...
 */
export abstract class KeyValueDBService<
		T extends ServiceContext = ServiceContext,
	>
	...
	implements IKeyValueDBService
{
	...
	private async _everyOnResetDatabase(any: unknown): Promise<boolean> {
		try {
			...
			this._kvDB = await OpenKeyValueDatabase(
				this.vault.getVaultName() + "-livesync-kv",
			);
			await delay(100);
		} catch (e) {
	...
	constructor(context: T, dependencies: KeyValueDBDependencies<T>) {
		...
		this.databaseEvents.onResetDatabase.addHandler(
			this._everyOnResetDatabase.bind(this),
		);
		this.appLifecycle.onSettingLoaded.addHandler(
			this._everyOnloadAfterLoadSettings.bind(this),
		);
		this.databaseEvents.onDatabaseInitialisation.addHandler(
			this._everyOnInitializeDatabase.bind(this),
		);
		this.databaseEvents.onUnloadDatabase.addHandler(
			this._onOtherDatabaseUnload.bind(this),
		);
		this.databaseEvents.onCloseDatabase.addHandler(
			this._onOtherDatabaseClose.bind(this),
		);
	}
