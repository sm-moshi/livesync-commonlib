 ...
 */
export abstract class ReplicatorService<
	...
	implements IReplicatorService
{
	...
	constructor(
		...
		this.settingService.onRealiseSetting.addHandler(
			this._initialiseReplicator.bind(this),
		);
		this.databaseEventService = dependencies.databaseEventService;
		this.databaseEventService.onResetDatabase.addHandler(
			this.disposeReplicator.bind(this),
		);
		this.databaseEventService.onDatabaseInitialisation.addHandler(
			this.disposeReplicator.bind(this),
		);
		this.databaseEventService.onDatabaseInitialised.addHandler(
			this.reinitialiseReplicator.bind(this),
		);
		this.databaseEventService.onDatabaseHasReady.addHandler(
			this.reinitialiseReplicator.bind(this),
		);
		this.appLifecycleService.onSuspending.addHandler(
			this.suspendReplication.bind(this),
		);
	}
	...
	private async _initialiseReplicator() {
		...
		if (replicatorType === this._replicatorType && this._activeReplicator) {
			...
			if (!(await this.onReplicatorInitialised())) {
				this._log(
					"Failed to initialise the replicator, onReplicatorInitialised reported some problems.",
				);
				this._activeReplicator = undefined;
				this._replicatorType = undefined;
				this._unresolvedErrorManager.showError(message, LOG_LEVEL_NOTICE);
			...
			const remoteTypeDisplay =
				replicatorType === RemoteTypes.REMOTE_COUCHDB
					? "CouchDB"
					: replicatorType;
			this._log(
				`Replicator (${remoteTypeDisplay}) initialised and activated`,
				LOG_LEVEL_VERBOSE,
			);
	 ...
	 * Not used currently.
	 */
	readonly onCloseActiveReplication = handlers<IReplicatorService>().anySuccess(
	...
	readonly getNewReplicator =
		handlers<IReplicatorService>().firstResult("getNewReplicator");

	readonly onReplicatorInitialised =
		handlers<IReplicatorService>().bailFirstFailure("onReplicatorInitialised");
