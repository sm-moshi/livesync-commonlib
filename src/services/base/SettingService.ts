...
import {
	decryptString,
	encryptString,
} from "../../encryption/stringEncryption";
	...
	implements ISettingService
{
	...
	private additionalSuffixOfDatabaseName() {
		...
		if (typeof suffix === "undefined") {
			this._log(
				"too early to get additionalSuffixOfDatabaseName, returning empty string",
			);
			return "";
		}
		return `-${suffix}`;
	...
	async saveSettingData() {
		...
		if (
			...
			settings.P2P_DevicePeerName.trim() !== ""
		) {
		...
		if (this.usedPassphrase == "" && !(await this.getPassphrase(settings))) {
			...
			if (settings.encrypt && settings.passphrase != "") {
				settings.encryptedPassphrase = await this.encryptConfigurationItem(
					settings.passphrase,
					settings,
				);
				settings.passphrase = "";
	 ...
	 */
	readonly onBeforeRealiseSetting =
		handlers<ISettingService>().bailFirstFailure("onBeforeRealiseSetting");

	/**
	 * Event triggered after the settings have been realised.
	 */
	readonly onSettingRealised =
		handlers<ISettingService>().bailFirstFailure("onSettingRealised");

	/**
	 * Event triggered to realise the settings.
	 */
	readonly onRealiseSetting =
		handlers<ISettingService>().bailFirstFailure("onRealiseSetting");
	 ...
	 */
	readonly suspendExtraSync =
		handlers<ISettingService>().all("suspendExtraSync");

	/**
	 * Suggest enabling optional features to the user.
	 */
	readonly suggestOptionalFeatures = handlers<ISettingService>().all(
	 ...
	 */
	readonly enableOptionalFeature = handlers<ISettingService>().all(
		"enableOptionalFeature",
	);
	readonly onSettingLoaded =
		handlers<ISettingService>().dispatchParallel("onSettingLoaded");
	readonly onSettingChanged =
		handlers<ISettingService>().dispatchParallel("onSettingChanged");
	readonly onSettingSaved =
		handlers<ISettingService>().dispatchParallel("onSettingSaved");
	...
	getPassphrase(settings: ObsidianLiveSyncSettings) {
		...
		> = {
			"": () => Promise.resolve("*"),
			LOCALSTORAGE: () =>
				Promise.resolve(this.getItem("ls-setting-passphrase") ?? false),
			ASK_AT_LAUNCH: () =>
				this.APIService.confirm.askString("Passphrase", "passphrase", ""),
		};
	...
	async encryptConfigurationItem(
		src: string,
		settings: ObsidianLiveSyncSettings,
	) {
		...
		if (passphrase === false) {
			...
			if (settings.encryptedCouchDBConnection) {
				...
				const decrypted = this.tryDecodeJson(
					await this.decryptConfigurationItem(
						settings.encryptedCouchDBConnection,
						passphrase,
					),
			...
			if (settings.encrypt && settings.encryptedPassphrase) {
				...
				const decrypted = await this.decryptConfigurationItem(
					encrypted,
					passphrase,
				);
				if (decrypted) {
	...
	async loadSettings(): Promise<void> {
		...
		if (
			isCloudantURI(this.settings.couchDB_URI) &&
		...
		if (this.getDeviceAndVaultName() == "") {
			if (this.settings.usePluginSync) {
				this._log(
					"Device name missing. Disabling plug-in sync.",
					LOG_LEVEL_NOTICE,
				);
