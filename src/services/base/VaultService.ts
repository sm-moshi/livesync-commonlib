	...
	implements IVaultService
{
	...
	getVaultName() {
		return (
			this.vaultName() +
			(this.settings.additionalSuffixOfDatabaseName
				? "-" + this.settings.additionalSuffixOfDatabaseName
				: "")
		);
	}
	 ...
	 */
	readonly isIgnoredByIgnoreFile = handlers<IVaultService>().anySuccess(
	 ...
	 */
	readonly isTargetFile =
		handlers<IVaultService>().bailFirstFailure("isTargetFile");
