 ...
 */
export abstract class AppLifecycleService<
	...
	implements IAppLifecycleService
{
	 ...
	 */
	readonly onLayoutReady =
		handlers<IAppLifecycleService>().bailFirstFailure("onLayoutReady");

	/**
	 * Event triggered when the plug-in is being initialized for the first time.
	 * This is only called once per plug-in lifecycle.
	 */
	readonly onFirstInitialise =
		handlers<IAppLifecycleService>().bailFirstFailure("onFirstInitialise");
	 ...
	 */
	readonly onReady =
		handlers<IAppLifecycleService>().bailFirstFailure("onReady");

	/**
	 * Event triggered to wire up necessary event listeners.
	 * This is typically called during the initialisation phase.
	 */
	readonly onWireUpEvents =
		handlers<IAppLifecycleService>().bailFirstFailure("onWireUpEvents");

	/**
	 * Event triggered when the plug-in is being initialised.
	 */
	readonly onInitialise =
		handlers<IAppLifecycleService>().bailFirstFailure("onInitialise");
	...
	readonly onSettingLoaded =
		handlers<IAppLifecycleService>().bailFirstFailure("onSettingLoaded");
	 ...
	 */
	readonly onLoaded =
		handlers<IAppLifecycleService>().bailFirstFailure("onLoaded");

	/**
	 * Scan for any startup issues that may affect the plug-in's operation.
	 */
	readonly onScanningStartupIssues = handlers<IAppLifecycleService>().all(
	 ...
	 */
	readonly onAppUnload =
		handlers<IAppLifecycleService>().dispatchParallel("onAppUnload");
	/**
	...
	readonly onBeforeUnload =
		handlers<IAppLifecycleService>().all("onBeforeUnload");
	 ...
	 */
	readonly onSuspending =
		handlers<IAppLifecycleService>().bailFirstFailure("onSuspending");

	/**
	 * Event triggered when the application is resuming from a suspended state.
	 */
	readonly onResuming =
		handlers<IAppLifecycleService>().bailFirstFailure("onResuming");

	/**
	 * Event triggered after the application has resumed from a suspended state.
	 */
	readonly onResumed =
		handlers<IAppLifecycleService>().bailFirstFailure("onResumed");
	 ...
	 */
	readonly getUnresolvedMessages =
		handlers<IAppLifecycleService>().dispatchParallel("getUnresolvedMessages");
}
