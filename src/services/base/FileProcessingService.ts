	...
	implements IFileProcessingService
{
	 ...
	 */
	readonly processFileEvent =
		handlers<IFileProcessingService>().anySuccess("processFileEvent");

	/**
	 * Process a file event item optionally, if any handler is registered.
	 * i.e., hidden files synchronisation or customisation sync.
	 */
	readonly processOptionalFileEvent =
		handlers<IFileProcessingService>().anySuccess("processOptionalFileEvent");

	/**
	 * Commit any pending file events that have been queued for processing.
	 */
	readonly commitPendingFileEvents =
		handlers<IFileProcessingService>().bailFirstFailure(
