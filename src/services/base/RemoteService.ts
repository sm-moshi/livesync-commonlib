...
} from "@lib/pouchdb/utils_couchdb";
...
import {
	...
	type LogFunction,
} from "@lib/services/lib/logUtils";
	...
	implements IRemoteService
{
	...
	constructor(context: T, dependencies: RemoteServiceDependencies) {
		...
		this._unresolvedErrors = new UnresolvedErrorManager(
	...
	async performFetch(
		...
		this._APIService.requestCount.value =
			this._APIService.requestCount.value + 1;
		...
		this._log(
			`[REQ] (${method}) ${localURL} -> ${response.status}`,
			LOG_LEVEL_DEBUG,
		);
	...
	async connect(
		...
		string | { db: PouchDB.Database<EntryDoc>; info: PouchDB.Core.DatabaseInfo }
	> {
				...
				if (opts?.body) {
					...
					if (opts_length > 1000 * 1000 * 10) {
						...
						if (isCloudantURI(uri)) {
							...
							this._log(
								"This request should fail on IBM Cloudant.",
								LOG_LEVEL_VERBOSE,
							);
				...
				try {
					...
					try {
						...
						if (ex instanceof TypeError) {
							...
							this.showError(
								`Failed to fetch by API. ${resp2.status}: ${msg}`,
								LOG_LEVEL_NOTICE,
							);
							return resp2;
						}
					...
					this._log(
						`HTTP:${method}${size} to:${localURL} -> failed`,
						LOG_LEVEL_VERBOSE,
					);
					const msg =
						ex instanceof Error ? `${ex?.name}:${ex?.message}` : ex?.toString();
					this.showError(
					...
					this._APIService.responseCount.value =
						this._APIService.responseCount.value + 1;
				}
