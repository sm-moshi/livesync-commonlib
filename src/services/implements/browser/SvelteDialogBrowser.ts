...
export class SvelteDialogBrowser<
	T,
	U,
	C extends ServiceContext = ServiceContext,
> extends BrowserSvelteDialogBase<T, U, C> {
	constructor(
		context: C,
...
export class BrowserSvelteDialogManager<
...
> extends SvelteDialogManagerBase<T> {
	override async openSvelteDialog<TT, TU>(
		...
		const dialog = new SvelteDialogBrowser<TT, TU, T>(
			this.context,
			this.dependents,
			component,
			initialData,
