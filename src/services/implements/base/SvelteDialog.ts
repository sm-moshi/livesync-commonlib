...
export type SvelteDialogManagerDependencies<
...
export type ComponentHasResult<T = any, U = any> = Component<
	HasSetResult<T> & HasGetInitialData<U>
>;
export type GuestDialogProps<T = any, U = any> = HasSetResult<T> &
	HasGetInitialData<U>;
...
export function getDialogContext<T = any, U = any>(): DialogContext<
	ServiceContext,
	T,
	U
> {
	return getContext<DialogContext<ServiceContext, T, U>>(
...
export abstract class SvelteDialogManagerBase<T extends ServiceContext> {
	...
	async openWithExplicitCancel<T, U>(
		component: ComponentHasResult<T, U>,
		initialData?: U,
	): Promise<T> {
		for (let i = 0; i < 10; i++) {
