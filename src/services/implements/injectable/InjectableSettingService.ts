...
export class InjectableSettingService<
  T extends ServiceContext,
> extends SettingService<T> {
  ...
  saveData = handlers<{
    saveData: (data: ObsidianLiveSyncSettings) => Promise<void>;
  }>().binder("saveData");
  loadData = handlers<{
    loadData: () => Promise<ObsidianLiveSyncSettings | undefined>;
  }>().binder("loadData");
}
