...
export class InjectableServiceHub<
	T extends ServiceContext = ServiceContext,
> extends ServiceHub<T> {
	...
	constructor(
		...
		this._databaseEvents =
			services.databaseEvents ?? new InjectableDatabaseEventService<T>(context);
		this._replicator = services.replicator;
		this._fileProcessing =
			services.fileProcessing ??
			new InjectableFileProcessingService<T>(context);
		this._conflict =
			services.conflict ?? new InjectableConflictService<T>(context);
		...
		this._tweakValue =
			services.tweakValue ?? new InjectableTweakValueService<T>(context);
		this._replication =
