...
export type AutoMergeResult = Promise<
	AutoMergeOutcomeOK | AutoMergeCanBeDoneByDeletingRev | UserActionRequired
>;
...
export class ConflictManager {
	...
	async getConflictedDoc(
		...
		try {
			const doc = await this.options.entryManager.getDBEntry(
				path,
				{ rev: rev },
				false,
				true,
	...
	async mergeSensibly(
		...
		LOOP_MERGE: do {
			...
			if (
				leftItem[0] == DIFF_EQUAL &&
				rightItem[0] == DIFF_EQUAL &&
			...
			if (
				leftItem[0] == DIFF_DELETE &&
				rightItem[0] == DIFF_DELETE &&
				leftItem[1] == rightItem[1]
			) {
				// when deleted evenly,
	...
	async mergeObject(
		...
		try {
			...
			const baseObj = { data: tryParseJSON(baseLeaf.data, {}) } as Record<
				string | number | symbol,
				any
			>;
			const leftObj = { data: tryParseJSON(leftLeaf.data, {}) } as Record<
				string | number | symbol,
				any
			>;
			const rightObj = { data: tryParseJSON(rightLeaf.data, {}) } as Record<
				string | number | symbol,
				any
			>;
	...
	async tryAutoMergeSensibly(
		...
		//Search
		const revFrom = await this.database.get<EntryDoc>(
			await this.options.entryManager.path2id(path),
			{
				revs_info: true,
			},
		);
		const commonBase =
			(revFrom._revs_info || []).filter(
				(e) =>
					e.status == "available" &&
					Number(e.rev.split("-")[0]) < conflictedRevNo,
			)?.[0]?.rev ?? "";
		let p = undefined;
		if (commonBase) {
			if (isSensibleMargeApplicable(path)) {
				...
				// can be merged.
				const result = await this.mergeObject(
					path,
					commonBase,
					test._rev!,
					conflictedRev,
	...
	async tryAutoMerge(
		...
		if (
			...
			const autoMergeResult = await this.tryAutoMergeSensibly(
				path,
				test,
				conflicts,
			);
