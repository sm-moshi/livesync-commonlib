...
export type HotPackProcessResult =
	(typeof HotPackProcessResults)[keyof typeof HotPackProcessResults];
...
export class ChunkManager {
	...
	emitEvent<K extends keyof ChunkManagerEventMap>(
		type: K,
		detail: ChunkManagerEventMap[K],
	): void {
	...
	constructor(options: ChunkManagerOptions) {
		...
		this.addListener(EVENT_CHUNK_FETCHED, this.onChunkArrivedHandler, {
			signal: this.abort.signal,
		});
		this.addListener(
			EVENT_MISSING_CHUNK_REMOTE,
			this.onMissingChunkRemoteHandler,
			{ signal: this.abort.signal },
		);
	...
	async readSingle(
		...
		try {
			...
			if (!isMissingError(error)) {
				...
				throw new LiveSyncError(`Failed to read chunk ${id}`, {
					status: 404,
					cause: error,
				});
			}
	...
	_readFromCache(
		...
		const cachedChunks = [...readIds]
			.map((id) => this.getCachedChunk(id))
			.filter((chunk) => chunk !== false);
	...
	async _readFromDatabase(
		...
		const results = await this.database.allDocs({
			keys: [...readIds],
			include_docs: true,
		});
	...
	_readPreloadedChunks(
		...
		for (const [id, chunk] of Object.entries(preloadedChunks) as [
			DocumentID,
			EntryLeaf,
		][]) {
			if (isChunkDoc(chunk)) {
	...
	async read(
		...
		return order.map((id) => resultMap.get(id) || false) as (
			| EntryLeaf
			| false
		)[];
	}
	...
	async write(
		...
		if (failed.some((res) => res.status !== 409)) {
			...
			throw new LiveSyncError(
				`Failed to write chunks: ${failed.map((res) => res.error).join(", ")}`,
				{
					status: 500,
				},
			);
		...
		if (conflictedChunkIDs.length > 0) {
			...
			const conflictedChunks = (
				await this.read(conflictedChunkIDs, { skipCache: false, timeout: 0 })
			).filter((chunk) => chunk !== false);
	...
	async transaction<T>(callback: () => Promise<T>): Promise<T> {
		...
		try {
			...
			if (this.concurrentTransactions === 0) {
				Logger(
					`All transactions completed. Performing stabilisation.`,
					LOG_LEVEL_VERBOSE,
				);
				...
				Logger(
					`Transaction completed. Remaining: ${this.concurrentTransactions}`,
					LOG_LEVEL_VERBOSE,
				);
			}
		}
