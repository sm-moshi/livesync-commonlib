...
} from "octagonal-wheels/promises";
import {
	StorageEventManager,
	type FileEvent,
} from "@lib/interfaces/StorageEventManager.ts";
import type {
	IAPIService,
	IVaultService,
...
export abstract class StorageEventManagerBase extends StorageEventManager {
	...
	protected get batchSaveMinimumDelay(): number {
		return (
			this.settings?.batchSaveMinimumDelay ??
			DEFAULT_SETTINGS.batchSaveMinimumDelay
		);
	}
	protected get batchSaveMaximumDelay(): number {
		return (
			this.settings?.batchSaveMaximumDelay ??
			DEFAULT_SETTINGS.batchSaveMaximumDelay
		);
	}
	get settings() {
	...
	constructor(dependencies: StorageEventManagerBaseDependencies) {
		...
		this._log = createInstanceLogFunction(
			"StorageEventManager",
			dependencies.APIService,
		);
	}

	abstract _saveSnapshot(
		snapshot: (FileEventItem | FileEventItemSentinel)[],
	): Promise<void>;
	abstract _loadSnapshot(): Promise<(FileEventItem | FileEventItemSentinel)[]>;

	// override if needed
	isFolder(
		file: UXFileInfoStub | UXInternalFileInfoStub | UXFolderInfo,
	): boolean {
	...
	async appendQueue(params: FileEvent[], ctx?: any) {
		...
		for (const param of params) {
			...
			const atomicKey = [0, 0, 0, 0, 0, 0]
				.map((e) => `${Math.floor(Math.random() * 100000)}`)
				.join("-");
			...
			if (type !== "INTERNAL") {
				...
				if (
					this.vaultService.isFileSizeTooLarge(size) &&
					(type == "CREATE" || type == "CHANGED")
				) {
					this._log(
	...
	protected bufferedQueuedItems = [] as (
		| FileEventItem
		| FileEventItemSentinel
	)[];
	...
	enqueue(newItem: FileEventItem) {
		...
		fireAndForget(() =>
			this._takeSnapshot().then(() => this.runQueuedEvents()),
		);
	}
		...
		const promises = [...this._waitingMap.entries()].map(([key, waitInfo]) => {
			return new Promise<void>((resolve) => {
				waitInfo.canProceed.promise
					.then(() => {
						this._log(
							`Processing ${key}: Wait for idle completed`,
							LOG_LEVEL_DEBUG,
						);
						// No op
					})
	...
	private _addWaiting(
		...
		if (elapsed >= maxDelay) {
			...
			this._log(
				`Processing ${key}: Batch save maximum delay already exceeded: ${event.type}`,
				LOG_LEVEL_DEBUG,
			);
		} else {
			this._log(
		...
		const waitInfo: WaitInfo = {
			...
			timerHandler: setTimeout(() => {
				this._log(
					`Processing ${key}: Batch save timeout reached: ${event.type}`,
					LOG_LEVEL_DEBUG,
				);
				this._proceedWaiting(key);
			}, nextDelay),
		...
		try {
			...
			if (previous) {
				...
				if (!isShouldBeCancelled && previous.type === fei.type) {
					...
					this._log(
						`Processing ${filename}: Cancelling previous waiting: ${fei.type}`,
						LOG_LEVEL_DEBUG,
					);
					isShouldBeCancelled = true;
				}
				...
				if (isShouldBeCancelled) {
					...
					this._log(
						`Processing ${filename}: Previous cancelled: ${fei.type}`,
						LOG_LEVEL_DEBUG,
					);
					isShouldBeCancelled = true;
				}
				if (!isShouldBeCancelled) {
					this._log(
						`Processing ${filename}: Waiting for previous to complete: ${fei.type}`,
						LOG_LEVEL_DEBUG,
					);
					this._proceedWaiting(waitingKey);
					this._log(
						`Processing ${filename}: Previous completed: ${fei.type}`,
						LOG_LEVEL_DEBUG,
					);
				}
			}
			...
			if (this.shouldBatchSave && !fei.skipBatchWait) {
				...
				this._log(
					`Processing ${filename}: Waiting for batch save: ${fei.type}`,
					LOG_LEVEL_DEBUG,
				);
				...
				if (!canProceed) {
					...
					this._log(
						`Processing ${filename}: Cancelled by new queue: ${fei.type}`,
						LOG_LEVEL_DEBUG,
					);
					return;
				}
	...
	async _restoreFromSnapshot() {
		...
		if (snapShot && Array.isArray(snapShot) && snapShot.length > 0) {
			...
			this._log(
				`Restoring storage operation snapshot: ${snapShot.length} items`,
				LOG_LEVEL_VERBOSE,
			);
			// Restore the snapshot
			// Note: Mark all items as skipBatchWait to prevent apply the off-line batch saving.
			this.bufferedQueuedItems = snapShot.map((e) => ({
				...e,
				skipBatchWait: true,
			}));
			this.updateStatus();
	...
	protected runQueuedEvents() {
		return skipIfDuplicated(
			"storage-event-manager-run-queued-events",
			async () => {
				do {
	...
	protected async handleFileEvent(queue: FileEventItem): Promise<any> {
		...
		const ret = await serialized(lockKey, async () => {
			if (queue.cancelled) {
				this._log(
					`File event cancelled before processing: ${file.path}`,
					LOG_LEVEL_INFO,
				);
				return;
			}
			if (queue.type == "INTERNAL" || file.isInternal) {
				await this.fileProcessing.processOptionalFileEvent(
					file.path as unknown as FilePath,
				);
			} else {
