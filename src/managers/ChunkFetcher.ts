...
export class ChunkFetcher {
	...
	get concurrency(): number {
		return (
			this.options.settings.concurrencyOfReadChunksOnline ||
			DEFAULT_SETTINGS.concurrencyOfReadChunksOnline
		);
	}
	...
	async requestMissingChunks(): Promise<void> {
		...
		try {
			...
			if (!fetched) {
				Logger(
					`No chunks were found for the following IDs: ${requestIDs.join(", ")}`,
				);
			...
			if (chunks.length === 0) {
				Logger(
					`No valid chunks were found for the following IDs: ${requestIDs.join(", ")}`,
				);
			}
			const missingIDs = requestIDs.filter(
				(id) => !chunks.some((chunk) => chunk._id === id),
			);
			try {
				if (chunks.length === 0) {
					return;
				...
				if (result.result === true) {
					...
					Logger(
						`Fetched chunks were stored successfully: ${chunks.length}`,
						LOG_LEVEL_VERBOSE,
					);
				} else {
					Logger(
				...
				Logger(
					`An error occurred while storing fetched chunks: ${error}`,
					LOG_LEVEL_VERBOSE,
				);
			} finally {
