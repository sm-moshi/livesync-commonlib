...
export class EntryManager {
	...
	isTargetFile(filenameSrc: string) {
		const file = filenameSrc.startsWith("i:")
			? filenameSrc.substring(2)
			: filenameSrc;
		...
		if (this.settings.syncOnlyRegEx) {
			...
			if (syncOnly.length > 0 && !syncOnly.some((e) => e.test(file)))
				return false;
		}
	...
	async prepareChunk(piece: string): Promise<GeneratedChunk> {
		...
		return {
			isNew: true,
			id: `${IDPrefixes.Chunk}${chunkId}` as DocumentID,
			piece: piece,
		};
	}
	...
	async getDBEntryFromMeta(
		...
		if (meta.type == "newnote" || meta.type == "plain") {
			...
			try {
				...
				const isChunksCorrectedIncrementally =
					this.settings.remoteType !== RemoteTypes.REMOTE_MINIO;
			...
			return (
				(await serialized("file:" + path, async () => {
					...
					if (!obj.type || (obj.type && obj.type == "notes")) {
						...
						Logger(
							`Entry removed:${path} (${obj._id.substring(0, 8)}-${r.rev})`,
						);
						return true;
	...
	async putDBEntry(note: SavingEntry, onlyChunks?: boolean) {
		...
		const data =
			note.data instanceof Blob ? note.data : createTextBlob(note.data);
		note.data = data;
		...
		const result = await this.chunkManager.transaction(async () => {
			...
			const flushBufferedChunks = async () => {
				...
				if (result.result === false) {
					Logger(
						`Failed to write buffered chunks for ${dispFilename}`,
						LOG_LEVEL_NOTICE,
					);
					return false;
				}
				...
				Logger(
					`Flushed ${writeBuf.length} (${writeChars}) chunks for ${dispFilename}`,
					LOG_LEVEL_VERBOSE,
				);
				return true;
			};
			const flushIfNeeded = async () => {
				if (bufferedSize > MAX_WRITE_SIZE) {
					if (!(await flushBufferedChunks())) {
						Logger(
							`Failed to flush buffered chunks for ${dispFilename}`,
							LOG_LEVEL_NOTICE,
						);
						return false;
					}
			...
			return (
				(await serialized("file:" + filename, async () => {
					...
					const r = await this.localDatabase.put<PlainEntry | NewEntry>(
						newDoc,
						{ force: true },
					);
					if (r.ok) {
		...
		Logger(
			`Document saved: ${dispFilename} (${result.id.substring(0, 8)}-${result.rev})`,
			LOG_LEVEL_VERBOSE,
		);
		return result;
	}
