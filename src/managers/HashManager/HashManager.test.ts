...
for (const hashAlg of Object.values(HashAlgorithms)) {
	...
	const managerWithEncryption = generateHashManager(
		generateSettings(hashAlg, "test"),
	);
	...
	Deno.test({
		...
		fn: async () => {
			...
			if (typeof hash !== "string") {
				throw new Error(
					`HashManager for ${hashAlg} did not return a string hash`,
				);
			}
			const piece2 = "test2";
			...
			if (hash === hash2) {
				throw new Error(
					`HashManager for ${hashAlg} returned the same hash for different inputs`,
				);
			}
			const hash3 = await manager.computeHash(piece);
			if (hash !== hash3) {
				throw new Error(
					`HashManager for ${hashAlg} returned different hashes for the same input`,
				);
			}
		},
	});
	Deno.test({
		...
		fn: async () => {
			...
			const hash1Encrypted =
				HashEncryptedPrefix +
				(await managerWithEncryption.computeHashWithEncryption(piece));
			...
			const hash1Plain =
				await managerWithEncryption.computeHashWithoutEncryption(piece);
			if (hash1Plain === hash) {
				throw new Error(
	...
	Deno.test({
		...
		fn: async () => {
			...
			if (hash !== CompatibilityPlain[hashAlg]) {
				throw new Error(
					`HashManager for ${hashAlg} returned incorrect hash for plain input: ${hash}`,
				);
			}
