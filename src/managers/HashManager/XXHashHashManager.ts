import {
	HashManagerCore,
	type HashManagerCoreOptions,
} from "./HashManagerCore.ts";
...
export class XXHash32RawHashManager extends XXHashHashManager {
	...
	computeHashWithEncryption(piece: string): Promise<string> {
		return Promise.resolve(
			(
				this.xxhash.h32Raw(new TextEncoder().encode(piece)) ^
				this.hashedPassphrase32 ^
				piece.length
			).toString(36),
		);
	}
	...
	computeHashWithoutEncryption(piece: string): Promise<string> {
		return Promise.resolve(
			(
				this.xxhash.h32Raw(new TextEncoder().encode(piece)) ^ piece.length
			).toString(36),
		);
	}
}
...
export class XXHash64HashManager extends XXHashHashManager {
	...
	computeHashWithEncryption(piece: string): Promise<string> {
		return Promise.resolve(
			this.xxhash
				.h64(`${piece}-${this.hashedPassphrase}-${piece.length}`)
				.toString(36),
		);
	}
	...
	computeHashWithoutEncryption(piece: string): Promise<string> {
		return Promise.resolve(
			this.xxhash.h64(`${piece}-${piece.length}`).toString(36),
		);
	}
}
...
export class FallbackWasmHashManager extends XXHashHashManager {
	...
	computeHashWithEncryption(piece: string): Promise<string> {
		return Promise.resolve(
			this.xxhash
				.h32(`${piece}-${this.hashedPassphrase}-${piece.length}`)
				.toString(36),
		);
	}
	...
	computeHashWithoutEncryption(piece: string): Promise<string> {
		return Promise.resolve(
			this.xxhash.h32(`${piece}-${piece.length}`).toString(36),
		);
	}
}
