...
import {
	...
	XXHash64HashManager,
} from "./XXHashHashManager.ts";
import {
	FallbackPureJSHashManager,
	PureJSHashManager,
	SHA1HashManager,
} from "./PureJSHashManager.ts";
import {
	HashManagerCore,
...
export class HashManager extends HashManagerCore {
	...
	async setManager(): Promise<boolean> {
		...
		throw new Error(
			`HashManager for ${this.settings.hashAlg} is not available`,
		);
	}
	...
	async processInitialise(): Promise<boolean> {
		if (await this.setManager()) {
			Logger(
				`HashManager for ${this.settings.hashAlg} has been initialised`,
				LOG_LEVEL_VERBOSE,
			);
			return true;
		}
		...
		throw new Error(
			`HashManager for ${this.settings.hashAlg} failed to initialise`,
		);
