...
export function __$checkInstanceBinding<T extends InstanceHaveOnBindFunction>(
	instance: T,
) {
	...
	const functionsOnBindFunctionMatch = onBindFunctionStr.match(
		/this\.(_[a-zA-Z0-9_]+)/g,
	);
	if (functionsOnBindFunctionMatch) {
		functionsOnBindFunction.push(
			...functionsOnBindFunctionMatch.map(
				(f) => `${thisName}.${f.replace(/this\./, "")}`,
			),
		);
	}
	...
	const missingInOnBind = [...setAll].filter(
		(e) => !setOfOnBindFunctions.has(e),
	);
	if (missingInThis.length > 0) {
		Logger(
			`[${thisName}] ⚠️ Missing functions in this: ${missingInThis.join(", ")}`,
		);
	}
	if (missingInOnBind.length > 0) {
		Logger(
			`[${thisName}] ⚠️ Missing functions in onBindFunction: ${missingInOnBind.join(", ")}`,
		);
	}
