...
import {
	type ContentSplitterOptions,
	type SplitOptions,
	MAX_CHUNKS_SIZE_ON_UI,
...
export abstract class ContentSplitterBase extends ContentSplitterCore {
	...
	getParamsFor(entry: SavingEntry): SplitOptions {
		const maxChunkSize = Math.floor(
			MAX_DOC_SIZE_BIN * ((this.options.settings.customChunkSize || 0) * 1 + 1),
		);
		...
		const blob =
			entry.data instanceof Blob ? entry.data : createTextBlob(entry.data);
		let useWorker = true;
		...
		const useSegmenter =
			this.options.settings.chunkSplitterVersion ===
			ChunkAlgorithms.V2Segmenter;
		return {
			blob,
			path,
			pieceSize,
