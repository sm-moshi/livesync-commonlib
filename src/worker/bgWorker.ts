...
import {
	encryptionOnWorker,
	encryptionHKDFOnWorker,
	handleTaskEncrypt,
} from "./bgWorker.encryption.ts";
...
export function encryptWorker(
	input: string,
	passphrase: string,
	autoCalculateIterations: boolean,
): Promise<string> {
	return encryptionOnWorker({
		type: "encrypt",
		input,
		passphrase,
		autoCalculateIterations,
	});
}

export function decryptWorker(
	input: string,
	passphrase: string,
	autoCalculateIterations: boolean,
) {
	...
	for (const inst of workers) {
		inst.worker.onmessage = ({ data }) => {
			...
			if (process.type === "split") {
				...
				process.type === "encryptHKDF" ||
				process.type === "decryptHKDF"
			) {
				handleTaskEncrypt(process, data);
			} else {
...
export function startWorker(
	data: Omit<EncryptArguments, "key">,
): EncryptProcessItem;
export function startWorker(
	data: Omit<SplitArguments, "key">,
): SplitProcessItem;
export function startWorker(
	data: Omit<EncryptArguments | SplitArguments | EncryptHKDFArguments, "key">,
): ProcessItem {
	const _key = key++;
