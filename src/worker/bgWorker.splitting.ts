...
const workerStreams = new Map<
	number,
	TransformStream<string | null, string | null>
>();
const writers = new Map<number, WritableStreamDefaultWriter<string | null>>();

const responseBuf = new Map<
	number,
	Map<number, string | SYMBOL_USED | SYMBOL_END_OF_DATA>
>();
...
export function handleTaskSplit(process: SplitProcessItem, data: any) {
	...
	if (!("result" in data)) {
		...
		const reportError =
			data.error || new Error("Unknown error in background splitting");
	...
	for (let i = 0; i <= max; i++) {
		...
		writerPromise = writerPromise.then(async () => {
			...
			if (!writer) {
				throw new Error(
					`Invalid writer for key ${key} in background splitting`,
				);
			}
